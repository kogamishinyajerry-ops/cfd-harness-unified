...
class DockerOpenFOAMSolverExecutor:
    ...
    # the density-based rhoCentralFoam supersonic-wedge path runs on the ESI v2312
    # runtime, NOT the Foundation-OF11 persistent `cfd-openfoam` container above.
    # It uses a FRESH `docker run --rm` ESI container sourcing `/openfoam/profile.rc`
    # — matching reports/showcase_aero/_w71a_wedge_probe/REPRODUCE.md verbatim — so
    # it disturbs no running container (~/CLAUDE.md hard rule) and is fully
    # independent of the OF11 incompressible runtime + its solver whitelist fence.
    ESI_WEDGE_IMAGE = "opencfd/openfoam-default:2312"
    ESI_PROFILE_RC = "/openfoam/profile.rc"
    # Frozen, validated wedge case definition (0/ constant/ system/; NO polyMesh —
    # regenerated by blockMesh). Single source of truth for the case geometry;
    # reused verbatim (no parametric generator) so the gate stays byte-faithful.
    _WEDGE_CASE_DEFINITION_REL = "reports/showcase_aero/_w71a_wedge_probe/case_definition"
    # Probe contract — geometry facts of the frozen case, mirroring the gold's
    # case_info.wedge_inputs (knowledge/gold_standards/wedge_oblique_shock.yaml).
    ...
    def execute(self, task_spec: TaskSpec) -> ExecutionResult:
        ...
        # 1.5. P4 V71.A (DEC-V61-234) — SUPERSONIC_WEDGE dispatches to a DEDICATED
        # density-based runner on the ESI v2312 image (rhoCentralFoam) that uses a
        # FRESH `docker run --rm` container + `/openfoam/profile.rc`, NOT the
        # Foundation-OF11 persistent `cfd-openfoam` container + put_archive path the
        # rest of execute() assumes. Short-circuit BEFORE the OF11 container connect
        # (below) so the wedge is fully independent of the incompressible runtime and
        # NEVER reaches the _OF11_INCOMPRESSIBLE_SOLVERS fence (~:895). Mirrors how
...
""",
    ...
    def _docker_run_esi_rm(
        self,
        host_dir: Path,
        command: str,
        timeout: int,
    ) -> Tuple[int, str]:
        """Run ``command`` in a FRESH ``--rm`` ESI v2312 container (DEC-V61-234).

        Bind-mounts ``host_dir`` at ``/work`` and sources the ESI profile, mirroring
        ``reports/showcase_aero/_w71a_wedge_probe/REPRODUCE.md`` VERBATIM::

            docker run --rm --entrypoint bash -v {host_dir}:/work \\
              opencfd/openfoam-default:2312 -c \\
              'source /openfoam/profile.rc >/dev/null 2>&1; cd /work && {command}'

        Uses the docker SDK (``detach`` → ``wait`` → ``logs`` → ``remove``) on a
        brand-new container, so NO already-running container is ever touched
        (~/CLAUDE.md hard rule). Returns ``(exit_code, combined_logs)``; on any
        docker-level error returns ``(-1, diagnostic)`` so the caller emits an
        honest BLOCK rather than a fabricated success. The ``--rm`` container we
        create is always force-removed in ``finally`` (it is OURS, not a shared one).
        """
        client = self._docker_client or docker.from_env()
        bash_cmd = (
            f"source {self.ESI_PROFILE_RC} >/dev/null 2>&1; "
            f"cd /work && {command}"
        )
        container = None
        try:
            container = client.containers.run(
                self.ESI_WEDGE_IMAGE,
                command=["-c", bash_cmd],
                entrypoint="bash",
            ...
            return -1, f"ESI --rm rhoCentralFoam run failed: {exc!r}"
        finally:
    ...
    def _execute_supersonic_wedge(
        ...
        """LIVE supersonic-wedge run via a FRESH ``--rm`` ESI v2312 rhoCentralFoam
        solve (DEC-V61-234, P4 V71.A — the ``foam_agent_adapter`` wiring that
        DEC-V61-233 explicitly DEFERRED). Density-based shock-capturing path, fully
        independent of the Foundation-OF11 incompressible runtime.

        Pipeline (matches ``reports/showcase_aero/_w71a_wedge_probe/REPRODUCE.md``):

          (a) stage the FROZEN, validated ``case_definition/`` (``0/ constant/
              system/``; NO polyMesh) into a fresh host work dir — reused verbatim,
              NO parametric generator (keeps the gate byte-faithful).
          (b) ONE ``docker run --rm`` ESI container chaining ``blockMesh &&
              checkMesh && rhoCentralFoam`` under ``set -e`` (the frozen
              controlDict's MachNo1/freestream/postShock/shockLine function objects
              run inline during the solve).
          (c) extract the oblique-shock QoIs via the EXECUTION-plane extractor
              (``src.wedge_oblique_shock_extractor`` — same plane, import allowed)
              and return ``ExecutionResult(success=True, is_mock=False, ...)``.

        Plane discipline (four-plane law): this Execution-plane runner does NOT
        import the Control-plane gate (``src.wedge_oblique_shock_gate``) — that would
        create an Execution⇄Control cycle. It EXECUTES + EXTRACTS; the authoritative
        oblique-shock PASS/FAIL verdict (``gate_wedge_against_gold``) is run by the
        Control-plane caller (the e2e test). ``success=True`` here means "a real
        rhoCentralFoam solve completed and produced extractable QoIs", NOT "the
        physics matches gold" — the latter is the gate's job.

        On ANY failure (docker unavailable, non-zero solver exit, missing
        postProcessing, non-physical extraction) returns ``self._fail(...)`` → an
        HONEST BLOCK. Never crashes the caller; never a silent misroute or a
        fabricated PASS.
        """
        # Execution→Execution import (allowed by the four-plane import-linter);
        # lazy so the adapter's module import graph is unchanged.
        ...
        case_id = re.sub(r"[^A-Za-z0-9_]", "_", (task_spec.name or "supersonic_wedge")).strip("_") or "supersonic_wedge"
        # Codex R0 P2: a UNIQUE work dir per run via mkdtemp — NEVER derive a fixed
        # path from task_spec.name and rmtree it, which would let a second/concurrent
        # launch erase a prior run's raw_output_path while a caller is still reading
        # it (corrupting the follow-up gate_wedge_against_gold + saved evidence).
        self._work_dir.mkdir(parents=True, exist_ok=True)
        work_dir = Path(tempfile.mkdtemp(prefix=f"{case_id}_", dir=str(self._work_dir)))
        raw_output_path = str(work_dir)
        try:
            # (a) stage the frozen, validated case definition into the fresh dir.
            src_def = Path(__file__).resolve().parent.parent / self._WEDGE_CASE_DEFINITION_REL
            if not src_def.is_dir():
                return self._fail(
                    f"supersonic-wedge case definition not found at {src_def} "
                    "(DEC-V61-234) — cannot stage the frozen rhoCentralFoam case; "
                    "honest BLOCK.",
                    time.monotonic() - t0,
                    raw_output_path=raw_output_path,
                )
            ...
            # (b) ONE fresh --rm ESI container: mesh + solve, chained under set -e so
            # the FIRST failing step aborts (honest BLOCK). blockMesh regenerates the
            # polyMesh; the controlDict function objects emit postProcessing inline.
            compound = "{ set -e; blockMesh; checkMesh; rhoCentralFoam; }"
            exit_code, run_log = self._docker_run_esi_rm(
                work_dir, compound, self._timeout
            )
            # Codex R0 P2: persist the solver log under raw_output_path so the
            # log-based consumers (TaskRunner._resolve_log_path, auto_verifier
            # _find_log_file) can find it — same `log.{solver}` convention the
            # OF11/CHT paths use (_docker_exec writes `log.{solver_name}`). Written
            # on BOTH success and failure; fail-tolerant (a non-writable dir must
            # not crash the run).
            try:
                (work_dir / "log.rhoCentralFoam").write_text(run_log, encoding="utf-8")
            except OSError:
                pass
            if exit_code != 0:
                return self._fail(
                    f"supersonic-wedge ESI rhoCentralFoam run failed "
                    f"(exit={exit_code}; DEC-V61-234). Log tail:\n{run_log[-2000:]}",
                    time.monotonic() - t0,
                    raw_output_path=raw_output_path,
                )

            # (c) Execution-plane QoI extraction (NOT the Control-plane gate).
            # Probe names/station are byte-faithful to the staged case + the gold's
            # wedge_inputs; the extractor fails closed on missing/non-physical data,
            # so success genuinely means "real run produced valid, extractable QoIs".
            try:
                qois = extract_wedge_qois(
                    work_dir,
                    x_shock_station=self._WEDGE_X_SHOCK_STATION,
                    shock_line_origin_y=self._WEDGE_SHOCK_LINE_ORIGIN_Y,
                    freestream_probe="freestream",
                    postshock_probe="postShock",
                    shock_line="shockLine",
                    shock_line_field="rho",
                )
            except (FileNotFoundError, WedgeShockExtractorError) as exc:
                return self._fail(
                    f"supersonic-wedge solve completed but QoI extraction failed "
                    f"({exc!r}; DEC-V61-234) — the run produced no valid "
                    "postProcessing artifacts; honest BLOCK (never a fabricated PASS).",
                    time.monotonic() - t0,
                    raw_output_path=raw_output_path,
                )
