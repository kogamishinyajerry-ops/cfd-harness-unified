...
- [ ] **M-V64A-VAL-FULL-3** 3rd FULL report · **candidate path post-B61 thermo-FPE**: 战略 pivot 切到 **incompressible canonical** (NASA TMR turbulent flat plate / Driver-Seegmiller backward-facing step / Moser DNS channel) 完全绕开 compressible gating · OR case_004 F-NEW-3 blade CAD fix incompressible 路径 · case_009 Sandia Flame D / case_016 multi-axis / case_006 solver-class swap 都已被 B61 重置为 multi-day work · commit: `_____`
...
当前 FULL validation reports (real solver convergence + literature delta):
                                                  **0 / 3 strict** (6 honest PARTIAL attempts: B53 case_016 thermo · B56/B57 case_004 case-spec ×2 · B59 case_006 thermo · B61 thermo-FPE fix v3 ×2 [case_016 + case_006] · NO inflation · 战略发现: thermo-FPE fix Layer 2 解决 → Layer 3 axes 浮现 · case_016 = 3-axis · case_006 = solver-class incompat · case_004 = blade CAD bug · Done #1 不是"fix one thing"而是 multi-layer engineering)
当前 canonical literature comparisons:            **2 / 3** (B56 NREL UAE Seq S + B59 Schmitt-Charpin · v3 fix-rerun 同 baseline 不计 net-new · 待 1 more canonical net-new · 推 Done #2 MET 还差 1)
当前 mesh convergence study (h/2 + h/4 monotonic): **1 / 1 ✓ MET** (B58 case_004 三档单调 Δ 8.47%)
当前 V63-A PARTIAL upgrade closure:               0 / ≥2 (case_004 PARTIAL v3 · case_016 PARTIAL v3 · case_006 PARTIAL v3 · 三 case 都 NOT upgraded · case_011 Path A 永久 PARTIAL 不计 upgrade)
当前 V63-A carry-over closure:                    **4 / ≥4 ✓ MET** (#2 first half mesh gen v2 B54 ✓ · #6 case_006 substrate v2 B55 ✓ · #4 D11 cross-val B60 ✓ · **#1 case_011 Path A rebadge B62 ✓**)
当前 V-row truth-capture rate:                    clause-1 over-met 3/2 · clause-2 ≥3/9 on ≥3 cases over-met 3/3 · ≥7/9 on 1 case carry-forward case_011 7/9 · V94 firm 3-case cross-validated (B60) · **V-candidate v3-new-1** limitTemperature fvOption substrate fix template [QUESTIONABLE 待 promotion to V101]
当前 Done dims MET:                               **2 / 6 ✓** (Done #3 B58 + **Done #5 B62 ✓ NEW**) · Done #2 + Done #6 distance 1 each · Done #1 + Done #4 都 0/target · charter premise 多次 revised: Done #1 multi-layer engineering 不是单 fix
```

最后更新时间：`2026-05-15 (V64-A B61 + B62 dual-dispatch landed · B61 thermo-FPE 系统修 PARTIAL v3 ×2 · case_016 + case_006 thermo 层 unblocked 但 Layer 3 axes [PIMPLE p-coupling / solver-class incompat] 暴露 · V-candidate v3-new-1 limitTemperature substrate fix template [QUESTIONABLE] · charter premise 第 3 次 revised: Done #1 multi-layer engineering not "fix one thing" · B62 case_011 Path A 用户裁决 PARTIAL rebadge · V63-A carry-over #1 CLOSED · **Done #5 3/4 → 4/4 ✓ MET (2nd Done dim MET in V64-A arc)** · 2 sub-DECs Notion synced 361c68942bed816b8bd3dfe3cce79752 + 361c68942bed8171b6aeecfe3e5fbd14 · 更新人：Claude Code Opus 4.7 session main)`
...
> **2026-05-15 B61 + B62 dual-dispatch landed** · 6 case-attempts honest 不掩盖 · **Done #5 4/4 ✓ MET** (B62 Path A) · charter premise 第 3 次 revised: 每修一层 thermo-FPE 暴露下一层 · Done #1 是 multi-layer engineering 不是 "fix one thing" · case_016 = 3-axis · case_006 = solver-class incompat · case_004 = blade CAD bug · 3 个 compressible 候选都已 re-tier 为 multi-day work.
>
> **关键判断**: Done #1 strict FULL 0/3 仍是最大 gap。剩下 4 个 Done dims (1/2/4/6)。Compressible path 已被 B61 证明 multi-layer · **战略 pivot 转向 incompressible**：
> 1. **case_004 blade CAD F-NEW-3 fix** = 已 locked root cause · 一行 section_wire() fix · incompressible simpleFoam 已经 PARTIAL v3 force-stable · 修了应该就 push Done #1 0→1/3
> 2. **新 incompressible canonical** = NASA TMR 湍流平板 / Driver-Seegmiller backward-facing step · canonical data 现成 · advisor stack 已支持 · 完全绕开 compressible gating axes
>
> **下一会话候选** (按 Done #1 strict FULL ROI 排序):
>
> 1. **M-V64A-CASE-004-BLADE-CAD-FIX** (Tier 2 · F-NEW-3 一行 section_wire() fix · CAD regen + 4th simpleFoam attempt · incompressible · 推 Done #1 0→1/3 strict · 同时 推 Done #4 PARTIAL→FULL upgrade 0→1/≥2 case_004 V63-A→V64-A FULL · 副产品: 推 Done #6 case_004 V-row 12→13+ rows)
> 2. **M-V64A-VAL-FULL-3-INCOMP** (Tier 2 · 新 substrate · NASA TMR turbulent flat plate vs Coles-Wadcock canonical OR Driver-Seegmiller backward-facing step vs experimental DB · incompressible canonical · 完全绕开 compressible gating · 推 Done #1 0→1/3 strict + Done #2 2→3/3 MET)
> 3. **M-V64A-CASE-016-PIMPLE-INIT** (Tier 2 · 攻 B61 Layer 3 case_016 · transient initialization OR PIMPLE pressure-coupling stabilization · multi-day work)
> 4. **M-V64A-CASE-006-SOLVER-SWAP** (Tier 2 · 攻 B61 Layer 3 case_006 · rhoSimpleFoam → rhoPimpleFoam transient + AMI fallback · multi-day work)
>
> **推荐并行**：**B63 = M-V64A-CASE-004-BLADE-CAD-FIX**（F-NEW-3 一行 section_wire() fix · incompressible 路径 · 推 Done #1 0→1/3 strict + Done #4 0→1/2）+ **B64 = M-V64A-VAL-FULL-3-INCOMP**（NASA TMR 平板 OR Driver-Seegmiller BFS · 完全新 incompressible canonical · 推 Done #1 + Done #2 MET 同步）。两个 brief 完全 scope-disjoint · B63 改 scripts/build_cad.py 单文件 + 重跑 case_004 · B64 建全新 case substrate + 跑 simpleFoam · 真并行. 若两者都 PASS → **Done #1 0→2/3 + Done #2 → 3/3 MET + Done #4 0→1/2** · V64-A close 路径打开.
