...
This addendum captures an observation. NO code change, NO test change, NO behavioral change. RETRO-V61-006 status remains: closed-with-four-addenda (1 = Codex R2 closeout, 2 = 40-CI-failure post-mortem + first GREEN signal, 3 = Opus direction-survey, 4 = this CI health sample finding).

---

## Addendum 5 · Governance rhythm refactor · 9 calendar→signal gates (2026-04-25T17:30 · Opus 4.7 ACCEPT_WITH_COMMENTS)

### Trigger

User authoritative feedback 2026-04-25T17:00 (verbatim):

> "我的开发速度完全不需要这种日期限制。我只想要按阶段、功能快速推进。"

This rejects the calendar-driven gate inheritance from Opus G-9 binding 2 and downstream OPS/ADR/RETRO date-anchoring. Notion @Opus 4.7 was prompted with a 9-gate enumeration + post-W5 roadmap request and returned **ACCEPT_WITH_COMMENTS** with **maturity 0.72 → 0.76 conditional** on the refactor landing.

### Opus self-attribution (root-cause)

Quoted verbatim from Opus 2026-04-25T17:30 reply:

> "线 A calendar-driven 倾向的根因 ≈ 70% 是 Opus G-9 自身 spec 起草习惯——我（Opus）写 binding 时下意识从人类 PR 治理文献继承了 deadline 范式 (sprint window / freeze period / SLA), 其实那些范式的合法性来自多人协调成本，单 user 拓扑下完全失去依据。20% 是 pre-commit/GitHub Actions cron 等工具默认值反向影响 spec 措辞。10% 是 Claude Code CLI 实施时的忠实落地，几乎无主动加码。**主责在 Opus 自身，需在后续 spec 起草时默认 signal-count、calendar 必须显式论证。**"

This is a methodology-level shift: **calendar gates require explicit justification going forward; signal-count gates are the default**.

### 9-gate refactor table (Opus authoritative)

| # | Gate | Calendar form (deprecated) | Signal-count form (active) |
|---|---|---|---|
| 1 | W4 toggle PR | ≤ 2026-05-11 | Gate #3 closes → fire (no independent deadline) |
| 2 | W5 default flip | ≤ 2026-05-19 | W4 merged ∧ ≥10 consecutive backend pytest CI runs all green ∧ A13/A18 dedup'd incidents = 0 in those 10 ∧ line A SOLE escape-tag commits = 0 in those 10 |
| 3 | Dogfood window | 2026-04-25 → 2026-05-09 (≥5d buffer) | ≥30 backend pytest CI runs ∧ ≥15 cross-track commits (A SOLE + B SOLE + SHARED) ∧ A13/A18 dedup'd incidents = 0 ∧ escape rate <20% |
| 4 | OPS expires | 2026-05-19 | W5 flip merged ∧ ≥5 post-flip CI runs all green → same PR retire |
| 5 | A18 §2.4 rollback trigger | 14-day rolling ≥3 incidents | last-30-CI-run rolling ≥3 dedup'd incidents |
| 6 | RETRO cadence | counter ≥ 20 | unchanged (already signal-count) |
| 7 | Codex audit cadence | "next risky-PR window" / weekly | (a) every risky-class PR + (b) every 10 merged PRs sample 1 non-risky for sweep (drift detection) |
| 8 | OPS §5#6 health threshold | 7-day rolling Green/Yellow/Red | last-20-CI-run rolling: Green ≥95%, Yellow 80-95%, Red <80%; STALE if 20 runs span >90 days |
| 9 | Phase 4 §5#6 known issue fix | "deferred to 5/19 PR" | bound to W5 PR composition (Gate #2 trigger), no independent timeline |

### Degradation conditions (each gate has explicit "trigger never fires" semantics)

Captured fully in `.planning/ops/2026-04-25_dual_track_plan.md` §3 / §5 / §7 (this commit) and `.planning/roadmaps/post_w5_roadmap.md` (companion file). Key principle: **trigger never firing IS the correct behavior** for many gates — Gate #4 (OPS expires) gracefully waits forever if W5 never flips; Gate #5 (A18 rollback) idles if no CI runs accumulate; Gate #1 (W4 toggle) is gated on dogfood close, which won't fire if the project pauses. Calendar would force inappropriate action; signal correctly waits.

### Calendar-retained gates: 0

Per Opus authority: "用户 '日期限制不必要' 反馈在线 A 范围内 ≈ 95% 正确——9 个 gates 全部可转 signal-count，无一项有合法 calendar 依据；剩余 5% 是元认知 sanity (是否睡够再 flip 之类的人类纪律), 但那不属于治理 gate 层，应该交给操作者自决而非写进 spec." Codex 5h/7d quota is a per-query constraint, NOT a governance gate.

### What this means for "5/9 review" and "5/11 / 5/19 deadlines"

**The "5/9 dogfood review" event no longer exists as a calendar event.** It is replaced by "Gate #3 close trigger evaluation" — fired when the 4-condition AND of Gate #3 is satisfied (≥30 runs ∧ ≥15 cross-track commits ∧ 0 dedup incidents ∧ escape rate <20%). The review template (`.planning/dogfood/2026-05-09_review_template.md`) is renamed and re-anchored in this commit to `.planning/dogfood/dogfood_close_review_template.md` with calendar references stripped.

Same for 5/11 (W4 toggle) and 5/19 (W5 flip + OPS retire) — these are now milestone-anchored, not date-anchored. They may fire earlier (if signal accumulates fast) or later (if project pauses). Both behaviors are correct.

### Maturity calibration update

| Snapshot | Maturity | Notes |
|---|---|---|
| Pre-Phase-1 (2026-04-25T15:10) | 0.62 (self-est) | At time of Opus initial OPS audit |
| Post-Phase-1-2-3 (2026-04-25T15:50) | 0.68 → 0.72 | Opus +0.04 conditional on direction 4 |
| Post-Phase-4 (2026-04-25T16:30) | 0.72 (held) | Direction 4 landed (dedup), conditional met |
| Post-Phase-6 doc-only (2026-04-25T17:30, this commit) | **0.76 conditional** | Opus +0.04 conditional on this 9-gates refactor + post-W5 roadmap landing as authority across spec text |

**Conditional**: the +0.04 gain is conditional on the spec text actually changing (this commit) and downstream gates being read in their new form by future evaluator runs. If a future commit silently re-introduces calendar form anywhere in line A scope, maturity drops back to 0.72 and a new RETRO addendum must capture the regression.

### Phase 6 landing scope

- This RETRO Addendum 5 — authority capture
- `.planning/ops/2026-04-25_dual_track_plan.md` — frontmatter `expires` field replaced with `expires_trigger`; §3 / §5 / §7 calendar references stripped and replaced with signal-count gates (gates 1-4, 8, 9)
- `.planning/dogfood/2026-05-09_review_template.md` → renamed to `dogfood_close_review_template.md` with anchor reframing
- `.planning/roadmaps/post_w5_roadmap.md` — NEW · 5-stage signal-driven plan (covers post-W5-1 through post-W5-5)
- `docs/methodology/ops_note_protocol.md` §5#6 — last-20-run window form (gate 8)
- `docs/methodology/pre_codex_self_review_checklist.md` — Codex audit cadence per gate 7
- `docs/adr/ADR-002-four-plane-runtime-enforcement.md` §5 timeline — milestone-anchored not date-anchored

(Code changes for gate 5 evaluator window and gate 8 OPS jq pipeline are NOT in this commit — they are gated on natural trigger windows per gate 5/8 semantics. Spec change today; code change when signal authorizes.)
