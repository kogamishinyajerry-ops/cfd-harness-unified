...
| Status | **`landed` 2026-05-13** (DEC-V61-198-sub-A4-face-orientation-advisor) — `ui/backend/services/geometry_ingest/face_orientation_advisor.py` + `ui/backend/tests/test_face_orientation_advisor.py` 9-test suite. Pure dict-consumer (mirrors A5 inlet_outlet_validator pattern) — caller pre-extracts `actual_face_normal` via topology-appropriate method (Z-axis: principal-normal dot-product; XY-axis: tilt-from-XY-plane signature), advisor `abs(dot)` compares against `expected_face_normal` or per-axis-median sibling consensus. Regression tests pin V79 38.000° + V87 21.979° ground-truth measurements |
