...
| Fix | **F1 (landed 2026-05-12)**: `case/system/snappyHexMeshDict` line 116 `APU_door level (3 4) → (2 3)`; `config/case.yaml` refinement section gained explicit `APU_door: [2, 3]` entry (SSOT alignment + comment trail of v28 rationale). Result: 49% skew face reduction, still FAIL on max-skewness. **F2/F3 (deferred · requires domain decision)**: Either (a) decide APU_door IS the inlet → drop body_1 from STL (merge into APU_door + retype naming.yaml from wall_adiabatic to mass_flow_inlet), OR (b) decide APU_door IS a closed wall → revert v26 snappyHexMeshDict edit (drop APU_door patch declaration, restore farfield aliasing for bay-air inlet at config/case.yaml domain layer). Neither (a) nor (b) is a 1-line fix — both require STL surgery in `01b_optimize_geom.py` + naming.yaml realignment + 08_bcs verification + full mesh rebuild |
| Status | F1 landed (partial improvement); **F2/F3 deferred to dedicated geometry session** — requires user domain decision on APU_door physical identity (wall vs inlet) which cannot be made from code alone |
| Reference case | APU bay case_002a · v27 → v28 mesh iteration · 2026-05-12 18:00 |
| Lesson | **Refinement-level tuning is a downstream lever; patch identity is the upstream constraint.** When max-skewness pegs at the 8.0 OpenFOAM wall AND a single patch holds 90%+ of skew faces, the problem is almost certainly NOT a refinement parameter but a **geometry interpretation conflict** — either the same physical region is represented by two overlapping STL bodies (sliver cells inevitable) or the patch type is contested (wall vs flow boundary). **Diagnostic pattern**: when checkMesh produces concentrated skew on a single patch, run the per-patch skew localization BEFORE tuning sHM controls. **Anti-pattern**: chasing skewness via `nSolveIter` / `nSmoothPatch` / `meshQualityControls.maxNonOrtho relaxed` when the source is two STL bodies trying to occupy the same physical region — relaxation lets bad cells through, smoothing can't move them out, and the solver later diverges anyway. **Mitigation**: the 07_check_mesh local heuristic ("→ nSolveIter ↑") is correct for diffuse skew but **misses the patch-concentration signature** entirely — future versions should detect when ≥80% of skew faces are on one patch and emit a different suggestion ("locate which STL bodies overlap with patch X; consider geometric merge/drop rather than sHM parameter tuning"). **Inheritance**: any industrial case with multiple STL bodies + hand-edited snappyHexMeshDict drift from generated template inherits this trap. **Counter row**: autonomous_governance: true (F1 landed in-session as 2-line config + comment; F2/F3 require domain decision · sub-DEC class if/when implemented) |
