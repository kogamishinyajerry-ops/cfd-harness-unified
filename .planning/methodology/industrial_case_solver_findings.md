...
| Status | **closed · field-validated** (3rd cross-topology validation); upgrades V21 from "open" to "open · refined hypothesis: case_005 failure is curved-geometry-specific, not all D1-class" |
| Reference case | case_004_nrel_phase_vi_mrf v1 advisor-validation 2026-05-08 (`evidence/v1_<ts>/defect_verification.json`) |
| Lesson | Cross-topology field validation accumulates evidence quickly. With 3 cases on the same advisor (3 = small but informative sample), **the structural axis of the failure is now observable**: axis-aligned planar geometry is a clean PASS regime; curved geometry surfaces an A2 scope gap. Pattern 6 application: V21's "advisor scope-narrowness" finding bifurcates into "geometry-class-conditioned scope" — A2 covers axis-aligned, A2-v2 should add curved. **Pillar 2 reinforcement**: each industrial case adds another point to the advisor falsification surface; the 3-case fan-out accelerates the scope-expansion sub-DEC's evidence base |
