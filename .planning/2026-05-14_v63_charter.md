# V63-A Industrial Scale-Up · Plan-File (Ratified 2026-05-14)

**Status**: `Ratified 2026-05-14 · V63-A selected · V63-B + V63-C → Alternatives Appendix`
**Plan SSOT**: `.planning/2026-05-14_v63_charter.md` (this file · renamed from `_draft` at B38)
**Charter DEC**: `.planning/decisions/2026-05-14_v63_charter_dec.md` (DEC-V63-A-charter Accepted 2026-05-14)
**Predecessor**: V62-A Advisor Stack Closure Arc (CLOSED 2026-05-14 · 6/6 Done dims MET · `DEC-V62-A-close` Accepted · `ARC-GOAL-V62-A-CLOSED.md` frozen)
**Successor placeholder**: V64 (TBD post-V63-A)
**Mode**: milestone-driven (no calendar · sustains V61-198 + V62-A precedent)
**Drafted**: 2026-05-14 (3-candidate exploration · parallel with B36 M-RADAR-V4 build)
**Ratified**: 2026-05-14 (user selected V63-A "Industrial Scale-Up" at V62 close · B38 commit chain)

> This is a **plan-file** (no frontmatter). The governance anchor is the
> charter DEC at `.planning/decisions/2026-05-14_v63_charter_dec.md`. This
> file remains the operational roadmap (Tier status, milestones, redirect
> conditions, counters); the DEC anchors `parent_dec` for V63-A sub-DECs.
> v2.3 round-1 rule: only Status=Accepted DECs sync to Notion · this
> plan-file itself does NOT sync (the charter DEC does).
>
> **V63-B (Frontend Activation)** and **V63-C (M6 Operationalization)** were
> considered and not selected at V62 close. Their full content (North Star,
> Done definition, Tier boards, dependency maps) is preserved verbatim in
> the **Alternatives Considered + Rejected** appendix at the bottom of
> this file. They remain available for future arc selection (V63-B-or-later /
> V63-C-or-later) without re-drafting.
...
## V63-A · "Industrial Scale-Up" · **SELECTED (Ratified 2026-05-14)**
...
## §5 · 4-dim comparison table (ratification evidence · 2026-05-14)

| 维度 | V63-A Industrial Scale-Up (SELECTED) | V63-B Frontend Activation | V63-C M6 Operationalization |
|---|---|---|---|
...
**Ratification rationale**: V63-A scored lowest engineering risk (2/5) AND highest V62-A asset reuse (5/5) — the dominant signals at V62 close given the freshly-landed stack assets. V63-B (1/5 dogfood distance · strongest UX value) and V63-C (5/5 strategic transform · M6 thesis operationalization) remain compelling forward-loaded options for V64-or-later arcs; both stay parked in the Alternatives Appendix for cheap re-selection.

---

## §6 · Recommendation history (preserved · pre-ratification record)

**Plan-author observation** (not prescription · authored 2026-05-14 pre-ratification): V63-A 是 V62-A 的自然路径延伸 · 风险最低、moat 最稳; V63-C 是战略价值最高、与 [feedback_claude_code_is_the_advisor] 真实命题对齐最强、也是商业 CAE AI 最难追的方向; V63-B 是用户 dogfood 距离最近但工程风险最高。三个都可行 · 选择取决于用户当下战略权重 (稳态产出 vs 用户可见性 vs M6 thesis 操作化)。

**Ratification (2026-05-14)**: user selected V63-A "Industrial Scale-Up". Kogami opt-in NOT invoked (per v2.3 user-judgment-only). V63-B + V63-C move to Alternatives Appendix.

---

## §7 · 沿用 V62-A 不变规则 (V63-A active · regardless of V63-B/V63-C in appendix)
...
## §8 · v2.3 governance 合规 (V63-A scope)

- V63-A 跨 ≥3 共享代码路径 (D11 advisor + D6 HTTP-wire route extension + V-series corpus extension + case_006 substrate) → **charter-level DEC required**. Anchored in `DEC-V63-A-charter` (Accepted 2026-05-14).
- V63-A 首个 sub-DEC (M-D11-DRAFT) 设 `parent_dec: V63-A-charter` (B39 candidate per task brief).
- Codex review: per-milestone scope per V63-A charter DEC §"v2.3 governance compliance":
  - M-D6-HTTP-WIRE (extends `routes/ai_review.py`) → 1-sync-trigger 命中 · Codex pre-merge 必走
  - Other Tier 1 milestones (D11 advisor source · D10 catalog · case substrate · validation reports) → Codex skip default
- Kogami opt-in (user-invoke only)
- Notion sync: session-end batch · only Status=Accepted DECs (this plan-file as plan-file does NOT sync; only `DEC-V63-A-charter` syncs · v2.3 round-1 rule)

---

## §9 · 下一步 (V63-A active)

1. **V63-A active**: ARC-GOAL.md initialized fresh at B38 (Tier 1 dispatch unblocked: M-D11-DRAFT + M-D6-HTTP-WIRE + M-D10-CATALOG parallel)
2. **First sub-DEC**: M-D11-DRAFT (D11 stl_face_label_validator · V94 face-label loss coverage) — candidate **B39** per task brief
3. **Codex review baseline**: M-D6-HTTP-WIRE is the first V63-A 1-sync-trigger milestone (route extension)
4. **Kogami opt-in**: user may invoke on any milestone per their judgment
5. **Notion sync**: pending session-end batch at B38 close (V62 close DEC + V63 charter DEC qualify · 12+ DECs queued)

---

## §A · Alternatives Considered + Rejected (V63-B + V63-C verbatim preserved)

Below preserves the V63-B + V63-C candidate sections verbatim from the
2026-05-14 draft for cheap re-selection in future V63-or-later arcs.
Neither was selected at V62 close (per §6 ratification record).

### A.1 · Candidate V63-B · "Frontend Activation" (NOT SELECTED)
...
### A.2 · Candidate V63-C · "M6 Operationalization" (Claude-Code-Session-As-Advisor) (NOT SELECTED)
...
**End of V63-A plan-file (ratified) + Alternatives Appendix.** V63-A
arc active. V63-B + V63-C preserved verbatim for cheap re-selection
without re-drafting in future arcs.
