# V74 Fleet Score · Iter 0

**Generated**: 2026-05-16T16:57:12Z  
**Commit**: `41dccd1e369d0aeb13df75e6c223a7e74adeb979`  
**Total (min one-vote-veto across 13 pillars)**: **8 / 100**  
**Weighted sum (informational)**: 112.68  
**Verdict**: PROCEED (multiple dims need attention)  
**Next-iter target dim**: `data_fidelity` (score=8)

## Per-Dim Scores (13 pillars)
...
| 11 | `interaction_polish` | 交互体验 | **92** | 0.07 | 🟢 high |
| 12 | `backend_integration` | 后端集成健康 | **96** | 0.06 | 🟢 high |
| 13 | `data_fidelity` | 数据保真度与可审计性 | **8** | 0.06 | 🔴 low |
...
- ✓ min() one-vote veto applied across all 13 pillars (no average masking)
...
- `lint`: 1
- `vitest`: 1
- `vitest_passed_count`: 446

**Evidence**:
- typecheck: PASS (tsc --noEmit clean)
- lint: PASS (eslint clean)
- vitest: PASS (446 tests · /tmp/v67c_vitest.log)
...
- `specs_pass_count`: 96
- `specs_total_count`: 96

**Evidence**:
- playwright: 96 specs PASS ≥17 threshold (FULL=60/60)
...
- `png_snapshot_count`: 44
- `viewport_mode_specs_pass`: 7

**Evidence**:
- visual baseline: 44/30 PNG files (FULL=30/30)
...
- motion polish: 14 transition usages (FULL=20/20)
- focus management: 49 ARIA/role/tabIndex usages (FULL=20/20)
- reduced-motion: 8 prefers-reduced-motion usages (FULL=15/15)
- axe-core: 3/3 PASS (pro-rated=12/20)

**Honest note**: V73 extension of V72 pillar 11 · 4 subscores → 5 · adds wcag_runtime via axe-core

## Agent: `backend_integration` · 后端集成健康

**Score**: 96 / 100 · Weight: 0.06

**Subscores**:
- `real_wired_surfaces`: 40
- `api_endpoint_coverage`: 16
...
**Evidence**:
- real-wired surfaces: 21 useQuery/api refs (FULL=40/40)
- API endpoints: 5/6 (pro-rated=16/20)
- graceful offline paths: 25 (FULL=20/20)
...
**Score**: 8 / 100 · Weight: 0.06

**Subscores**:
- `run_id_visible`: 0
- `gold_delta_visible`: 8
- `audit_package_downloadable`: 0
- `byte_repro_hash_visible`: 0
- `gold_delta_row_count`: 1
- `provenance_hash_count`: 0

**Evidence**:
- gold-delta rows: 1/3 (pro-rated=8/25)

**Failures**:
- TopBar lacks data-testid='topbar-run-id' (V74.3)
- audit-package download wire missing from v3 (V74.5)
- 0 provenance-hash chips in TruthChain (V74.3)
