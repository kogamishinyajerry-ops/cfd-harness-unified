# V77 Fleet Score · Iter 2

**Generated**: 2026-05-16T20:10:08Z
**Commit**: `0de5e89b16d1bf11d4e9013930774e3f393977db`
**Total (min one-vote-veto across 16 pillars)**: **50 / 100**
**Weighted sum (informational)**: 125.37
**Verdict**: PROCEED (lift lowest dim)
**Next-iter target dim**: `quality` (score=50)

## Per-Dim Scores (16 pillars)

| # | Agent | Dim | Score | Weight | Status |
|---|---|---|---|---|---|
| 1 | `quality` | 代码质量 | **50** | 0.12 | 🟡 mid |
| 2 | `physics` | 物理/数值稳定 | **100** | 0.12 | ✅ PASS-99 |
| 3 | `ux` | 使用手感 | **87** | 0.15 | 🟡 mid |
| 4 | `visualization` | 可视化追踪 | **100** | 0.15 | ✅ PASS-99 |
| 5 | `smoke` | 端到端 pipeline | **54** | 0.08 | 🟡 mid |
...
**Score**: 50 / 100 · Weight: 0.12

**Subscores**:
- `typecheck`: 0
- `lint`: 0
- `vitest`: 1
- `vitest_passed_count`: 459

**Evidence**:
- vitest: PASS (459 tests · /tmp/v67c_vitest.log)

**Failures**:
- typecheck: 1 TS errors · see /tmp/v67c_typecheck.log
- lint: 3 errors · see /tmp/v67c_lint.log
...
- `specs_pass_count`: 117
- `specs_total_count`: 122

**Evidence**:
- playwright: 117 specs PASS ≥17 threshold (FULL=60/60)
...
- backend HTTP /api/cases probe: PASS (port=50385 · 3853 bytes)
...
**Failures**:
- frontend build FAILED · tail: > tsc -b && vite build  src/pages/workbench/v3/__tests__/SolverStream.contract.test.tsx(8,32): error TS6133: 'vi' is declared but its value is never read.
- typecheck: 1 TS errors
- lint: 1 errors
...
**Subscores**:
- `sse_event_stream`: 25
- `residual_live_update`: 25
- `solver_state_stream`: 25
- `inflight_residual_display`: 25
- `sse_hook_count`: 22
- `sse_status_count`: 1
...
**Evidence**:
- sse_event_stream: 22 hook refs + 1 status testid (FULL=25/25)
- residual_live_update: 8 per-var testids (FULL=25/25)
- solver_state_stream: 1 state-badge testid (FULL=25/25)
- inflight_residual_display: 1 ticker testid (FULL=25/25)
