# V76 Fleet Score · Iter 0

**Generated**: 2026-05-16T19:11:56Z
**Commit**: `0de5e89b16d1bf11d4e9013930774e3f393977db`
**Total (min one-vote-veto across 15 pillars)**: **0 / 100**
**Weighted sum (informational)**: 125.00
**Verdict**: PROCEED (multiple dims need attention)
**Next-iter target dim**: `visualization_fidelity` (score=0)

## Per-Dim Scores (15 pillars)
...
| 15 | `visualization_fidelity` | 三维可视化保真度 | **0** | 0.06 | 🔴 low |
...
- ✓ min() one-vote veto applied across all 15 pillars (no average masking)
...
- `specs_pass_count`: 114
- `specs_total_count`: 114

**Evidence**:
- playwright: 114 specs PASS ≥17 threshold (FULL=60/60)
...
- `png_snapshot_count`: 60
- `viewport_mode_specs_pass`: 7

**Evidence**:
- visual baseline: 60/30 PNG files (FULL=30/30)
...
- backend HTTP /api/cases probe: PASS (port=51751 · 3853 bytes)
...
**Evidence**:
- real-wired surfaces: 41 useQuery/api refs (FULL=40/40)
- API endpoints consumed: 8 distinct (FULL=20/20)
- graceful offline paths: 40 (FULL=20/20)
- integration tests: 9 refs (FULL=20/20)

**Honest note**: V76 pillar 12 · useQuery ≥30 forces viz queries to land · template-expression regex closes V73.4/V74.3/V75.1 literal-testid trap
...
- `load_fallback`: 0
- `mount_count`: 0
- `camera_widget_count`: 0
- `legend_count`: 0
- `fps_count`: 0
- `fallback_count`: 0

**Failures**:
- 0 vtk-canvas-mounted testids in v3 (V76.1)
- 0 vtk-camera-reset / vtk-axes-widget testids in v3 (V76.3)
- 0 vtk-color-legend testid in v3 (V76.4)
- 0 vtk-fps-indicator testid in v3 (V76.4)
- 0 vtk-webgl-fallback testid in v3 (V76.5)
