# V78 Fleet Score · Iter 4 · TIGHTENED scoring

**Generated**: 2026-05-18T06:07:52Z
**Commit**: `0de5e89b16d1bf11d4e9013930774e3f393977db`
**Total (min one-vote-veto across 16 pillars)**: **65 / 100**
**Weighted sum (informational)**: 125.80
**Verdict**: PROCEED (lift lowest dim)
**Next-iter target dim**: `smoke` (score=65)
...
|---|---|---|---|---|---|
| 1 | `quality` | 代码质量 | **70** | 0.12 | 🟡 mid |
| 2 | `physics` | 物理/数值稳定 | **100** | 0.12 | ✅ PASS-99 |
| 3 | `ux` | 使用手感 | **76** | 0.15 | 🟡 mid |
| 4 | `visualization` | 可视化追踪 | **100** | 0.15 | ✅ PASS-99 |
| 5 | `smoke` | 端到端 pipeline | **65** | 0.08 | 🟡 mid |
...
| 16 | `real_time_solver_observability` | 实时求解器可观察性 | **80** | 0.06 | 🟡 mid |
...
**Score**: 70 / 100 · Weight: 0.12

**Subscores**:
- `typecheck`: 0
- `lint`: 1
- `vitest`: 1
- `vitest_passed_count`: 786

**Evidence**:
- lint: PASS (eslint clean)
- vitest: PASS (786 tests · /tmp/v67c_vitest.log)

**Failures**:
- typecheck: 3 TS errors · see /tmp/v67c_typecheck.log
...
**Score**: 76 / 100 · Weight: 0.15

**Subscores**:
- `flow_completion`: 49
- `latency_band`: 12
- `no_blocker_clicks`: 15
- `specs_pass_count`: 125
- `specs_total_count`: 153

**Evidence**:
- playwright: 125/153 (49/60 pro-rated by exact pass rate)
- no-blocker: 0 click-intercepted / timeout signals

**Failures**:
- V78 tightened: 125/153 specs PASS · need 100% for FULL
...
- `png_snapshot_count`: 92
- `viewport_mode_specs_pass`: 7

**Evidence**:
- visual baseline: 92 PNG files (FULL=30/30 · V78 raised threshold to 76)
...
- backend HTTP /api/cases probe: PASS (port=59276 · 3853 bytes)
...
**Failures**:
- frontend build FAILED · tail: src/pages/workbench/v3/components/canvas/VtkCanvasV3.tsx(153,24): error TS2339: Property 'setCameraPreset' does not exist on type 'ViewportKernel'. src/pages/workbench/v3/components/canvas/VtkCanvasV3.tsx(156,24): error TS2339: Property 'setCameraPreset' does not exist on type 'ViewportKernel'. src/pages/workbench/v3/components/canvas/VtkCanvasV3.tsx(159,24): error TS2339: Property 'setCameraPreset' does not exist on type 'ViewportKernel'.
- typecheck: 3 TS errors
...
- steadiness regimes: 56 (≥2 V70 threshold MET)
- BC types: 485 (≥10 V70 threshold MET)
...
- v3 palette: sand-coral #b78b65 referenced 10 times (≥3 MET)
...
- motion polish: 20 transition usages (FULL=20/20)
- focus management: 82 ARIA/role/tabIndex usages (FULL=20/20)
- reduced-motion: 21 prefers-reduced-motion usages (FULL=15/15)
...
**Evidence**:
- real-wired surfaces: 80 useQuery/api/SSE refs (FULL=40/40)
- API endpoints consumed: 10 distinct (FULL=20/20)
- graceful offline paths: 58 (FULL=20/20)
- integration tests: 17 refs (FULL=20/20)
...
- URL state resumability: 17 sync sites (FULL=25/25)
...
**Evidence**:
- sse_event_stream: 23 hook refs + 1 status testid (FULL=20/20)
...
**Failures**:
- backend SSE E2E FAILED · see /tmp/v78_solver_e2e.log
