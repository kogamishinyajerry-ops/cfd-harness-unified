# V68-C Fleet Score · Iter 1

**Generated**: 2026-05-16T10:16:04Z  
**Commit**: `d0eabcb5811db0fc553eb3f1a21f7644dc20ecce`  
**Total (min one-vote-veto)**: **73 / 100**  
**Weighted sum (informational)**: 95.70  
**Verdict**: PROCEED (lift lowest dim)  
**Next-iter target dim**: `functional` (score=73)
...
| 4 | `visualization` | 可视化追踪 | **92** | 0.2 | 🟢 high |
| 5 | `smoke` | 端到端 pipeline | **100** | 0.1 | ✅ PASS-99 |
| 6 | `functional` | 功能完整度 | **73** | 0.1 | 🟡 mid |
...
- whitelist count: 11 (≥11 V68-C threshold MET)
...
- `specs_pass_count`: 37
- `specs_total_count`: 37

**Evidence**:
- playwright: 37 specs PASS ≥7 threshold (FULL=60/60)
- latency: PASS (all specs within timeout)
- no-blocker: 0 click-intercepted / timeout signals

**Honest note**: V68-C tightened · ≥9 specs PASS for FULL · pro-rated below

## Agent: `visualization` · 可视化追踪

**Score**: 92 / 100 · Weight: 0.2

**Subscores**:
- `render_success_rate`: 40
- `mode_switch_correctness`: 30
- `visual_diff_baseline`: 22
- `png_snapshot_count`: 12
- `viewport_mode_specs_pass`: 7

**Evidence**:
- visual baseline: 12/16 PNG files (pro-rated=22/30)
- viz+truth specs: 9/9 PASS (render=40/40)
- viewport-mode: 7 PASS ≥4 threshold (FULL=30/30)

**Failures**:
- visual baseline incomplete: 12/6 PNG snapshot files (need ≥16 for V68-C close close)

**Honest note**: V68-C tightened · ≥16 PNG snapshot files + ≥4 viewport-mode specs required for full score · pro-rated below threshold
...
- backend HTTP /api/cases probe: PASS (port=55613 · 3853 bytes)
...
**Honest note**: V68-C added live HTTP probe (uvicorn boots + /api/cases responds 200); per-iter smoke still excludes OpenFOAM heavy run (dogfood_loop.py reserved for arc-close gate)

## Agent: `functional` · 功能完整度

**Score**: 73 / 100 · Weight: 0.1

**Subscores**:
- `landed_sub_dec_count`: 3
- `landed_sub_dec_total`: 4
- `done_dim_met`: 5
- `done_dim_total`: 7

**Evidence**:
- LANDED: 2026-05-16_v68c_sub_v68c1_material_card_wiring.md
- LANDED: 2026-05-16_v68c_sub_v68c2_advisor_offline_fallback.md
- LANDED: 2026-05-16_v68c_sub_v68c3_apu_bay_catalog_entry.md
- Done dims MET: 5/7 (from .planning/V68C_ARC_GOAL.md)
