...
def section_wire(r_m: float, chord_m: float, twist_deg: float) -> cq.Wire:
    ...
    # F-NEW-3 fix (B57→this sub-DEC · 2026-05-15): chord-axis convention bug.
    # Old: theta = radians(twist+pitch). At theta=0 the section's chord aligns
    # with +x (rotation axis) → FEATHERED blade → drag-driven torque → Cp > Betz.
    # NREL/TP-500-29955 Table B-1 + Phase VI design convention: zero twist + zero
    # pitch means chord lies IN the rotor plane (yz plane for rotation axis = ±x).
    # +90° offset reorients reference chord from +x (axial) to +y (tangential),
    # which is in-plane. Positive twist then tilts LE toward +x (upstream into
    # freestream), matching NREL Phase VI twist sign convention (+20° at root).
