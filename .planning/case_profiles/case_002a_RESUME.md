...
**Last session**: 2026-05-13 (F4b solver-execution validation; user early-stop at step 2689 / endTime=3000)
**Prior session**: 2026-05-12 (full-day mesh quality investigation arc · v27 → v33 → reverted to v32)
**Status**: **F4b CLOSED POSITIVE** — buoyantSimpleFoam ran cleanly for 2689 SIMPLE iters (10.4 h ExecutionTime, 10.95 h wall) on v32 polyMesh (max_skew 6.87 / 20 skew faces) with sHM maxInternalSkewness 3→8 relaxed for SSOT. Zero FATAL / FPE / Killed signatures. p_rgh init residual plateaued 0.39-0.54 (statistically stable, NOT numerically steady) because of missing apu_intake outlet patch — physics issue, not mesh. T bounded 316-855 K throughout. Sedimented as **V84** in `.planning/methodology/industrial_case_solver_findings.md`. Mesh quality state from prior session unchanged: v32 = 3.10M cells, max_skewness 6.875, 20 skew faces, 3 failed checks. The "solver runs on max_skew 6.87 mesh" claim is now empirically validated end-to-end from Claude Code chat window.
