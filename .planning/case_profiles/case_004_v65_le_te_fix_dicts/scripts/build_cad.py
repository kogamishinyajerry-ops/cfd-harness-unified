...
TIP_PITCH_DEG = 0.0  # NREL Phase VI Sequence S baseline (per NREL/TP-500-29955 Table 3-2). v1 used 3.0 which mismatched Seq S; corrected B57.
...
def section_wire(r_m: float, chord_m: float, twist_deg: float) -> cq.Wire:
    ...
    # F-NEW-3 fix (B57→V64-A B63 · 2026-05-15): chord-axis convention bug.
    # F-NEW-3.1 fix (V65-A B80 · 2026-05-16): tangential LE/TE off-by-180°.
    # v4 (B63): theta = +π/2 + radians(twist+pitch). +π/2 offset got chord IN the
    # rotor plane (correct), but the LE landed on the WRONG tangential side —
    # blade ran trailing-edge-first into freestream. Empirical: |M_x|=272 N·m vs
    # canonical 787 N·m (35% magnitude · M_x SIGN wrong per right-hand rule).
    # v5 (B80): negate BOTH the offset AND the twist sign → theta = -π/2 - radians(...).
    # This flips the tangential side without disturbing the in-plane chord
    # constraint. Expected: M_x sign flips negative; |M_x| rises 272→500-700;
    # Cp 0.12→0.25-0.40 (marginal-FULL or FULL band).
    ...
    for x_norm, y_norm in blended_section_coords(r_mm):
        ...
        # Rotate about the local radial axis (=z): see F-NEW-3 fix note above.
