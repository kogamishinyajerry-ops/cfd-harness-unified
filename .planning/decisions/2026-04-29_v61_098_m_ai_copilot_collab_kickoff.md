...
status: Accepted (2026-04-30 · gating swap from human-CFDJerry-smoke → Claude-Code-automated-smoke per CFDJerry workflow directive 2026-04-30 "visual smoke由你Claude code负责，CFDJerry无法被自动化触发，移除出自动化开发工作流" · scripts/smoke/dogfood_loop.py exit 0 [§4a LDC cube full loop · §4c channel full loop · §7 negatives · frontend Vite boot probe] · Codex 7-round arc COMPLETE [Steps 2+3 round 3 APPROVE at commit 67b0465; Steps 6+7a round 2 RESOLVED at commit b3e1720; Step 7b round 2 RESOLVED at commit 0abdd74] · Kogami NOT triggered per V61-094 P2 #1)
...
implementation_paths:
  - ui/backend/services/case_annotations/ (NEW package · YAML reader/writer + face_id hash + revision counter · per spec_v2 §A1 · reconciled 2026-04-29 from earlier `case_setup/` placeholder per surface_scan_2026-04-29.md §4.1)
  - ui/backend/schemas/ai_action.py (NEW · AIActionEnvelope + UnresolvedQuestion Pydantic schemas · per spec_v2 §A2)
  - ui/backend/services/ai_actions/ (NEW package · envelope wrapper · setup_bc_with_annotations() concrete action · per spec_v2 §A3)
  - ui/backend/routes/case_annotations.py (NEW · GET/PUT `/api/cases/<id>/face-annotations` · per spec_v2 §A4)
  - ui/backend/routes/case_solve.py (MODIFIED · `setup-bc` route returns envelope shape under `?envelope=1`; legacy SetupBcSummary path preserved for backward-compat · per spec_v2 §A5)
  - ui/backend/tests/test_case_annotations.py (NEW · face_id stability under polyMesh regen + round-trip + revision)
  - ui/backend/tests/test_ai_action_schema.py (NEW · envelope schema validation + blocked/uncertain/confident transitions)
