...
notion_sync_status: synced 2026-04-27 (https://www.notion.so/34fc68942bed81faa70ce66980396ffd)
...
3. **`--tools ""` 未来语义变化**：Anthropic 可能在未来版本改 `--tools ""` 行为（如 "" → 等价于 default）。**Mitigation**（v3 W4 dependency-triggered fix · per 项目"禁用日期/调度门控"核心原则 + user 2026-04-27 ratification）：Q1 canary 改为**dependency-triggered**（不是 calendar-driven）—— `kogami_invoke.sh` wrapper 在每次调用前对比 `claude --version` 与 `.planning/governance/claude_version_baseline.txt`，version 不同即触发 Q1 canary（5 runs），pass 则更新 baseline，fail 则 abort wrapper 并打印 Tier 2 escalation 提示。这是真正的 dependency gating（CLI 升级 = 唯一真实失效信号），不依赖日历。
...
- Q1 canary 测试（dependency-triggered: claude CLI version change）任意一次 fail（subprocess output 含 canary token）
...
**Counter provenance interpretation (canonicalization · per W3 blind-control Kogami P2-2 finding)**：
- **Interpretation B (STATE.md `last_updated` field = SSOT) 是 canonical going forward**，per V61-086 provenance precedent
- 任何未来 DEC 的 `autonomous_governance_counter_v61` 字段值必须基于 STATE.md `last_updated` 中最新计入的 counter 推算
- 中间"silent advances"（DEC frontmatter 不写 counter 但 autonomous_governance: true）不影响 SSOT；唯一计算来源是 STATE.md
- 这关闭了 V61-080/081/082/FORENSIC-FLAKE 的 silent-advance 期间的 A vs B 模糊状态
...
**作为 regression test 长期作用**：dependency-triggered（`claude --version` 变化时由 wrapper 自动跑），任何命中 → §3.6 Tier 2 升级触发。Calendar/cron 调度被禁（项目"禁用日期/调度门控"原则，user ratified 2026-04-27）。
...
| ID | 风险 | Mitigation |
|---|---|---|
| R-1 | --tools "" 在未来 claude 版本语义变化 | Q1 canary regression test (dependency-triggered, runs in wrapper before each Kogami invocation if `claude --version` changed)；任何 leak 立即 Tier 2 升级 |
...
- ❌ 不修改 Codex 触发规则
- ❌ 不引入新 counter
- ❌ **Hard boundary (files)**：任何修改 P-1/P-1.5/P-2/P-2.5/P-3/P-4/P-5 的 DEC 禁止由 Kogami 自身 review 批准；必须 user 显式 ratification + Codex APPROVE
- ❌ **Hard boundary (meta DECs · per W3 blind-control Kogami P2-1 finding)**：以下 meta 路径**也**禁止 Kogami 自审：(a) DEC-V61-087 本身的修改；(b) 任何 `parent_dec` 包含 DEC-V61-087 的 DEC；(c) 任何 subject matter 修改 Kogami isolation contract / counter rules / trigger rules / Tier 1-2 boundary 的 DEC，不论文件路径触及哪些。这关闭了 framing-bypass 漏洞（"我们只是修 §3 不改 P-1.5"仍属 self-modification）
