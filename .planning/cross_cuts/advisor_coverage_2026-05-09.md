...
| **D6_advisor** — `extra_body_advisor.py` | V55 (case_016 first injection 2026-05-11) · case_018 still dispatched | **1 sediment + 1 dispatched · LANDED 2026-05-14 with [QUESTIONABLE] pending case_018** | ~290 LOC source + 10-test suite (0.05s) | **DONE — closes V61-198 §5.2 D-class literal-counter waiver + satisfies V62-A Done dim #4 (D-class LANDED ≥1); 3 detection paths: (a) `unregistered_body` critical via STL-vs-manifest cross-check (V55 case_016 sighting class) · (b) `body_in_fluid_region` warning via solid-bbox ⊂ fluid-bbox AABB containment · (c) `undeclared_inclusion` warning via solid ⊂ solid without `declared_inclusions` entry. Pure dict-consumer pattern (mirrors A4/A5/A8/A10 sibling pattern · 0 file IO · 0 CAD-library dep). M-D6-PROMOTE Tier 2 milestone met (DEC-V62-A-sub-D6 · single-case land per V61-198 §5.2 + A2 v1 precedent; status remains [QUESTIONABLE 2026-05-14] until case_018 sediment lands).** |
...
| D6 (extra body in fluid) | 016, 018 = 2× sedimented(1)+dispatched(1) | **`extra_body_advisor.py` LANDED 2026-05-14** (DEC-V62-A-sub-D6 · single-case land per V61-198 §5.2 + A2 v1 precedent) | V55 closed with [QUESTIONABLE] pending case_018; closes V61-198 §5.2 D-class waiver |
