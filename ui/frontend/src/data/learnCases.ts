...
export const LEARN_CASES: LearnCase[] = [
  {
    ...
    glossary_zh: [
      ...
      { term: `stream function ψ`, definition: `2D 不可压流里定义 U_x = ∂ψ/∂y, U_y = −∂ψ/∂x 的标量场。ψ 的等值线就是流线（粒子轨迹），ψ 的极值点就是涡心。DEC-V61-050 batch 2 (2026-04-23) 落地 ui/backend/services/psi_extraction.py，用 ψ = ∫₀^y U_x dy' 从 simpleFoam 体数据重建 ψ；/flow-fields/lid_driven_cavity/stream_function.png 已换成真实解，主涡中心与 Ghia Re=100 匹配到 0.23% ψ_min 误差。`},
      ...
      { term: `Ghia Table I / II / III`, definition: `Ghia, Ghia & Shin 1982 (JCP 47, 387) 核心数据表。Table I 列了不同 Re 下 x=0.5 垂直中线上 17 个 y 对应的 u/U_lid 值。Table II 列了 y=0.5 水平中线上 17 个 x 对应的 v/U_lid 值。Table III 列了主涡 + 底部角涡 BL/BR 的中心位置 + ψ 强度。DEC-V61-050 (2026-04-23) 把这三张表全部接进 audit comparator：Table I → D2-D4, Table II → D6, Table III 主涡行 → D7, Table III BL/BR 行 → D8（但 D8 信号低于 ψ 数值地板，标 PARTIAL）。`},
