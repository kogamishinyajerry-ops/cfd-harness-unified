...
export const LEARN_CASES: LearnCase[] = [
  {
    ...
    solver_setup_zh: `<strong>Why this choice</strong>: simpleFoam (稳态 SIMPLE 算法) + turbulenceProperties=laminar. SIMPLE 用 pressure-correction 迭代耦合动量和连续性方程，每步松弛更新 U 和 p；时间项不出现，所以没有 CFL 约束，收敛由 under-relaxation factors 控制 (U≈0.7, p≈0.3)。Re=100 层流腔体流本质是稳态，用稳态 solver 比瞬态更直接、更省算力。<strong>Main alternative</strong>: pimpleFoam (瞬态 PIMPLE) 可以跑时间推进直到 d/dt≈0，但对 Re=100 纯粹浪费——唯一合理用瞬态的情况是 Re≥5000 时涡脱开始非稳态。<strong>When it breaks</strong>: URF 调太激进（p>0.5 或 U>0.9）会让 residual 震荡不收敛；调太保守（p<0.1）会跑几千步才够深。<strong>What to inspect</strong>: ControlDict 的 endTime (SIMPLE 里是 iteration count)、fvSolution 的 solver (对 p 用 GAMG + tolerance=1e-6)、fvSchemes 的 div(phi,U) 用 linearUpwind 保正守性。`,
    mesh_strategy_zh: `<strong>Why this choice</strong>: blockMesh 生成结构化均匀方格网格，ncx=ncy=40 (~1600 cells) 的分辨率在教学场景下够分辨主涡 + 底部二级涡；Ghia 1982 的 129×129 是 benchmark-grade (reference)，40×40 是 fast-run + 同一比较方法的 teaching-grade。<strong>Main alternative</strong>: O-grid / C-grid 对腔体没有好处（几何是直角方腔，uniform 最省心）；边界层 graduation 在 Re=100 可以省（边界层相对腔尺度不薄）。如需 scan Re→1000 则要 grading（角点速度梯度变陡）。<strong>When it breaks</strong>: 网格太粗（<20² cells）会抹掉二级涡；Ghia 的 primary_vortex_location 被你拖偏 5% 以上就是这个症状。<strong>What to inspect</strong>: Mesh tab 的 grid convergence sweep (mesh_20 → mesh_160) 显示 u_centerline deviation 递减到 <1% 时可认为网格独立——这是 Richardson extrapolation 的 visual 验证。`,
    boundary_conditions_zh: `<strong>Why this choice</strong>: 四面 BC 的对称性很重要。顶壁 (movingWall) fixedValue U=(1,0,0) + p zeroGradient：moving lid 是问题的唯一 momentum source。左右底三面 (fixedWalls) fixedValue U=(0,0,0) + p zeroGradient：no-slip wall 把动量传到内部流体。压力在一个点 (referencePressure) 固定为 0 作为参考——不可压 N-S 里只有 pressure gradient 有意义，绝对值要锚定。<strong>Main alternative</strong>: 用 cyclic 周期 BC 做无限长 cavity → 变成 Couette flow (完全不同的问题)；movingWall 换成 slip wall → 没有 lid-driven momentum，流体纹丝不动。<strong>When it breaks</strong>: 忘记 referencePressure 导致 p 漂移不收敛；把 movingWall 写成普通 wall 会被 OpenFOAM 提示 "U = fixedValue not allowed on wall"；顶角 (U=1 和侧壁 U=0 同一个点) 是数学奇点，但 OpenFOAM 默认 cell-center 值不撞到这个点，物理上是 paper-thin discontinuity。<strong>What to inspect</strong>: constant/polyMesh/boundary 里 patch type 必须是 wall，0/U 里 movingWall patch 的 type 是 fixedValue，参考压力在 fvSolution::referencePressure block 里。`,
    observable_extraction_zh: `<strong>Why this choice</strong>: u_centerline 沿 x=0.5 竖线从 y=0 到 y=1 抽 17 点 U_x 值——这是 Ghia 1982 Table II 的确切 sampling scheme，所以 comparator 可以点对点比。选 x=0.5 (腔体中线) 是因为它穿过主涡中心，信号幅值最大、分辨率对它最敏感。<strong>Main alternative</strong>: v_centerline 沿 y=0.5 采 V_y 是 Ghia Table I 的数据 (本 case 也支持，但教学上 u_centerline 已经够)；primary_vortex_location 用 streamFunction ψ=0 等值线找主涡中心 (x_c, y_c) 是第二道交叉验证（正 Ghia Re=100 给 (0.6172, 0.7344)）。<strong>When it breaks</strong>: sample 线取错位置 (x=0.25 而不是 0.5) → 幅值偏低；extractor 对 uniform mesh 容易，non-uniform mesh 要 OpenFOAM sample utility 插值，如果 interpolationScheme 选 cellPointFace 就会在 boundary 附近产生 artifact。<strong>What to inspect</strong>: src/foam_agent_adapter.py 里的 sample 调用 (lookupCellsContaining + ValuePointer)，comparator 输出的 L2 + max deviation——L2 低但 max 高往往是单点 outlier (常在 y=1 顶角附近)。`,
    workflow_steps_zh: [
      { step: `准备 case 骨架`, command: `foamNewCase ldc_re100 && cd ldc_re100`, detail: `从 $FOAM_TUTORIALS/incompressible/icoFoam/cavity 拷贝模板，得到 0/ constant/ system/ 三个目录，U 和 p 的 zero-state 由此开始。`},
      { step: `写 blockMeshDict`, command: `$EDITOR system/blockMeshDict`, detail: `设置 vertices 为单位方腔 (0 0 0)-(1 1 0.1)，blocks 的 cells (40 40 1) 给 1600 cells 的 teaching 分辨率；boundary 里把顶面标 movingWall、其余三面 fixedWalls、前后 empty (伪 2D)。`},
      { step: `生成网格`, command: `blockMesh`, detail: `成功日志结尾会打印 6 patches + nCells=1600。用 checkMesh 扫一遍 aspect ratio / non-orthogonality，方腔均匀网格通常 max non-ortho=0，无告警。`},
      { step: `设置 BC 与 Re`, detail: `编辑 0/U：movingWall type=fixedValue value=(1 0 0)，fixedWalls type=fixedValue value=(0 0 0)。0/p 全部 zeroGradient。constant/transportProperties 里 nu 设为 0.01 让 Re = U·L/ν = 1·1/0.01 = 100。`},
      { step: `启动 solver`, command: `simpleFoam > log.simpleFoam &`, detail: `controlDict 里 endTime=2000、deltaT=1 (SIMPLE 里 deltaT 代表迭代步)、writeInterval=500；同时 foamLog log.simpleFoam 拿到 residuals 文件，U+p 应在 <1500 步 drop 到 1e-5 以下。`},
      { step: `抽取 u-centerline`, command: `postProcess -func sampleDict`, detail: `sampleDict 在 x=0.5 从 y=0 到 y=1 取 17 点 U_x，输出 postProcessing/sampleDict/<t>/line_x05_U.xy，提交给 comparator.py 和 Ghia 1982 Table II 对齐。L2<1.5% 视为 PASS。`},
    ],
    troubleshooting_zh: [
      { symptom: `Residuals 震荡不收敛（每 100 步大跳跃）`, likely_cause: `under-relaxation factors 太激进，p>0.5 或 U>0.9，SIMPLE 的 pressure 解被过度 amplified。`, fix: `fvSolution::relaxationFactors 改为 p=0.3 U=0.7，重跑。若仍震荡，检查 fvSchemes 是否把 div(phi,U) 设成 upwind (Gauss upwind) 稳定版。`},
      { symptom: `u_centerline 幅值对但位置偏 5%`, likely_cause: `网格太粗未分辨主涡中心位置，primary_vortex_location 被拖向腔体几何中心。`, fix: `blockMeshDict 把 cells 从 40×40 加到 80×80 或 100×100，重跑；Mesh tab 的 convergence sweep 会显示幅值趋近 Ghia。`},
      ...
      { symptom: `solver 初始几步 p residual = NaN`, likely_cause: `遗漏 fvSolution::referencePressure 或 referenceCell，不可压 N-S 的 pressure 无约束 → drift。`, fix: `fvSolution 的 SIMPLE 块加 pRefCell 0 + pRefValue 0，重跑。NaN 通常第一步就出现，检查 log.simpleFoam 前 10 行。`},
      { symptom: `二级涡（底角）消失`, likely_cause: `网格分辨率不足（<30×30 cells），二级涡尺度小于 cell 尺度被数值耗散抹平。`, fix: `加密到 80×80 或更密，同时把 fvSchemes 的 div(phi,U) 从 upwind 换到 linearUpwind 降低 numerical diffusion。`},
    ],
    benchmark_lineage_zh: {
