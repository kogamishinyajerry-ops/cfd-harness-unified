#!/usr/bin/env bash
# V76 Fleet Aggregator · 15 pillars
# Runs:
#   - V69 7 + V70 3 + V72 interaction_polish (v74_fleet)
#   - V73 backend_integration (v76_fleet · ≥30 threshold + template regex)
#   - V74 data_fidelity (v74_fleet)
#   - V75 resumability_observability (v75_fleet)
#   - V76 NEW visualization_fidelity (v76_fleet)
# Usage: bash scripts/governance/v76_fleet/score_all.sh <iter_number>
# Output: .planning/scores/V76_iter_${N}.md

set -o pipefail
cd "$(git rev-parse --show-toplevel)" || exit 1

iter="${1:-0}"
out=".planning/scores/V76_iter_${iter}.md"
mkdir -p "$(dirname "$out")"

ts=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
sha=$(git rev-parse HEAD 2>/dev/null || echo "no-git")

agents=(quality physics ux visualization smoke functional stability cfd_breadth novice_onboarding industrial_ui interaction_polish backend_integration data_fidelity resumability_observability visualization_fidelity)
results_dir=$(mktemp -d)
trap "rm -rf $results_dir" EXIT

echo ">>> V76 Fleet · Iter $iter · $ts · commit $sha" >&2

for a in "${agents[@]}"; do
  echo ">>> running agent: $a" >&2
  # V76: visualization_fidelity (NEW pillar 15) uses v76_fleet/
  # backend_integration uses v76_fleet/ (extended threshold ≥30 + template regex)
  # resumability_observability uses v75_fleet/ (unchanged from V75)
  # data_fidelity / interaction_polish uses v74_fleet/ (unchanged)
  # everything else reuses v71_fleet/ scripts
  if [ "$a" = "visualization_fidelity" ] || [ "$a" = "backend_integration" ]; then
  ...
  elif [ "$a" = "data_fidelity" ] || [ "$a" = "interaction_polish" ]; then
...
md = []
md.append(f"# V76 Fleet Score · Iter {iter_n}\n")
md.append(f"**Generated**: {ts}  ")
md.append(f"**Commit**: `{sha}`  ")
md.append(f"**Total (min one-vote-veto across 15 pillars)**: **{min_score} / 100**  ")
...
md.append("## Per-Dim Scores (15 pillars)\n")
...
md.append("- ✓ min() one-vote veto applied across all 15 pillars (no average masking)\n")
