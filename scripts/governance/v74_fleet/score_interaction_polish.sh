...
PYEOF
  ...
  # V74 · raised threshold from 3 to 5 surfaces (Steps 1+2+3+4+5)
  if [ "${total:-0}" -ge 5 ] && [ "${passed:-0}" -eq "${total:-0}" ]; then
    wcag_score=20
    evidence+=("axe-core runtime a11y: ${passed}/${total} PASS · 0 WCAG violations across 5 surfaces (FULL=20/20)")
  elif [ "${passed:-0}" -gt 0 ]; then
    wcag_score=$(( passed * 20 / 5 ))
