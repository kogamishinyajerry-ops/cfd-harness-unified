#!/usr/bin/env bash
# V74 Fleet Aggregator
# Runs 13 score agents:
#   V69 7 + V70 3 + V72 interaction_polish + V73 backend_integration + V74 NEW data_fidelity
# Collects JSON, writes markdown report.
# Usage: bash scripts/governance/v74_fleet/score_all.sh <iter_number>
# Output: .planning/scores/V74_iter_${N}.md

set -o pipefail
cd "$(git rev-parse --show-toplevel)" || exit 1

iter="${1:-0}"
out=".planning/scores/V74_iter_${iter}.md"
mkdir -p "$(dirname "$out")"

ts=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
sha=$(git rev-parse HEAD)

agents=(quality physics ux visualization smoke functional stability cfd_breadth novice_onboarding industrial_ui interaction_polish backend_integration data_fidelity)
results_dir=$(mktemp -d)
trap "rm -rf $results_dir" EXIT

echo ">>> V74 Fleet · Iter $iter · $ts · commit $sha" >&2

for a in "${agents[@]}"; do
  echo ">>> running agent: $a" >&2
  # V74: data_fidelity (NEW pillar 13) uses v74_fleet/
  # interaction_polish + backend_integration use v74_fleet/ (extended)
  # everything else reuses v71_fleet/ scripts
  if [ "$a" = "data_fidelity" ] || [ "$a" = "interaction_polish" ] || [ "$a" = "backend_integration" ]; then
...
agents_order = ["quality", "physics", "ux", "visualization", "smoke", "functional", "stability",
                ...
                "backend_integration", "data_fidelity"]
data = {}
...
md = []
md.append(f"# V74 Fleet Score · Iter {iter_n}\n")
md.append(f"**Generated**: {ts}  ")
md.append(f"**Commit**: `{sha}`  ")
md.append(f"**Total (min one-vote-veto across 13 pillars)**: **{min_score} / 100**  ")
...
md.append("## Per-Dim Scores (13 pillars)\n")
...
md.append("- ✓ min() one-vote veto applied across all 13 pillars (no average masking)\n")
