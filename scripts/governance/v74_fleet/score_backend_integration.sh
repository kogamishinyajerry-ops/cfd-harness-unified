...
wired_score=0
# V74 threshold: ≥18 (was ≥12 in V73) · forces per-ref completeness +
# run_id wire + audit-package wire to land. V73 close counts 21 ·
# V74 work must keep it ≥18 even after the ribbon refactor.
if [ "${wired:-0}" -ge 18 ]; then
  wired_score=40
  evidence+=("real-wired surfaces: ${wired} useQuery/api refs (FULL=40/40)")
elif [ "${wired:-0}" -gt 0 ]; then
  wired_score=$(( wired * 40 / 18 ))
  if [ "$wired_score" -gt 40 ]; then wired_score=40; fi
  evidence+=("real-wired surfaces: ${wired}/18 (pro-rated=${wired_score}/40)")
else
...
endpoint_score=0
# V74 threshold: ≥6 (was ≥5 in V73) · forces 1 more endpoint to land
if [ "${endpoints:-0}" -ge 6 ]; then
  endpoint_score=20
  evidence+=("API endpoints consumed: ${endpoints} distinct (FULL=20/20)")
elif [ "${endpoints:-0}" -gt 0 ]; then
  endpoint_score=$(( endpoints * 20 / 6 ))
  if [ "$endpoint_score" -gt 20 ]; then endpoint_score=20; fi
  evidence+=("API endpoints: ${endpoints}/6 (pro-rated=${endpoint_score}/20)")
else
