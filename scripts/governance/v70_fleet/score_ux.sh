...
PYEOF
  ...
  if [ "$total" -gt 0 ]; then
    # V70 (further tightened): ≥13 specs PASS for FULL flow_score=60 · pro-rated otherwise
    if [ "$passed" -ge 13 ]; then
      flow_completion_score=60
      evidence+=("playwright: ${passed} specs PASS ≥13 threshold (FULL=60/60)")
    else
      flow_completion_score=$(( passed * 60 / 13 ))
      evidence+=("playwright: ${passed}/13 PASS threshold (pro-rated=${flow_completion_score}/60 · ${passed}/${total} total)")
      failures+=("ux below V70 threshold: ${passed}/13 specs PASS (need ≥13 for V70 close · was ≥11 for V69)")
    fi
