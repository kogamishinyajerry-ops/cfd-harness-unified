#!/usr/bin/env bash
# V70 Fleet Aggregator
# Runs 10 score agents (V69 7 + V70 3 new), collects JSON, writes markdown report.
# Usage: bash scripts/governance/v70_fleet/score_all.sh <iter_number>
# Output: .planning/scores/V70_iter_${N}.md

set -o pipefail
cd "$(git rev-parse --show-toplevel)" || exit 1

iter="${1:-0}"
out=".planning/scores/V70_iter_${iter}.md"
mkdir -p "$(dirname "$out")"

ts=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
sha=$(git rev-parse HEAD)

agents=(quality physics ux visualization smoke functional stability cfd_breadth novice_onboarding industrial_ui)
results_dir=$(mktemp -d)
trap "rm -rf $results_dir" EXIT

echo ">>> V70 Fleet · Iter $iter · $ts · commit $sha" >&2

for a in "${agents[@]}"; do
  echo ">>> running agent: $a" >&2
  bash "scripts/governance/v70_fleet/score_${a}.sh" > "$results_dir/${a}.json" 2> "$results_dir/${a}.stderr.log"
...
agents_order = ["quality", "physics", "ux", "visualization", "smoke", "functional", "stability",
                "cfd_breadth", "novice_onboarding", "industrial_ui"]
data = {}
...
md = []
md.append(f"# V70 Fleet Score · Iter {iter_n}\n")
md.append(f"**Generated**: {ts}  ")
md.append(f"**Commit**: `{sha}`  ")
md.append(f"**Total (min one-vote-veto across 10 pillars)**: **{min_score} / 100**  ")
...
md.append("## Per-Dim Scores (10 pillars)\n")
...
md.append("- ✓ min() one-vote veto applied across all 10 pillars (no average masking)\n")
