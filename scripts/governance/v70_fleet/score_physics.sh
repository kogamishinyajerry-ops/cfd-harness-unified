...
if [ "$canonical_count" -ge 30 ]; then
  canonical_pass=1
  evidence+=("canonical eval files (individual): ${canonical_count} (≥30 V70 threshold MET)")
else
  failures+=("canonical eval files (individual): ${canonical_count} (need ≥30 for V70 close · was ≥20 for V69)")
fi
