...
PYEOF
  ...
  if [ "$total" -gt 0 ]; then
    # V68-C (tightened): ≥9 specs PASS for FULL flow_score=60 · pro-rated otherwise
    if [ "$passed" -ge 9 ]; then
      flow_completion_score=60
      evidence+=("playwright: ${passed} specs PASS ≥7 threshold (FULL=60/60)")
    else
      flow_completion_score=$(( passed * 60 / 9 ))
      evidence+=("playwright: ${passed}/9 PASS threshold (pro-rated=${flow_completion_score}/60 · ${passed}/${total} total)")
      failures+=("ux below V68-C threshold: ${passed}/7 specs PASS (need ≥9 for V68-C close)")
    fi
...
print(json.dumps({
  ...
  "evidence": ev,
  "failures": fa,
  "honest_note": "V68-C tightened · ≥9 specs PASS for FULL · pro-rated below"
