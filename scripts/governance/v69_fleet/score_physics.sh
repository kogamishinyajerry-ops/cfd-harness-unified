#!/usr/bin/env bash
# V69 Fleet Agent #2: Physics Validator (tightened vs V68-B)
# V69 criteria:
...
#   - whitelist_count(25) · /api/cases LIST returns ≥11 (10 baseline + case_002a from V69.3)
...
# V69 whitelist count assertion · target ≥11 after case_002a entry lands
whitelist_pass=0
whitelist_count=0
...
if [ "$whitelist_count" -ge 11 ]; then
  whitelist_pass=1
  evidence+=("whitelist count: ${whitelist_count} (≥11 V69 threshold MET)")
else
  failures+=("whitelist count: ${whitelist_count} (need ≥11 for V69 close · case_002a entry pending)")
fi

# V69 NEW · canonical advisor eval set count (≥20 INDIVIDUAL case files for FULL).
# Charter §3 mandates individual case files only — batched B104/B105 docs do
# NOT count. Strict per user mandate: "ls 看到 E01..E20 全部 20 个 case 文件
# （不再有 batched）".
canonical_count=0
canonical_pass=0
if [ -d ".planning/evals/canonical" ]; then
  canonical_count=$(grep -lE "^eval_case_id:" .planning/evals/canonical/*.md 2>/dev/null | wc -l | tr -d ' ')
fi
canonical_count=${canonical_count:-0}
if [ "$canonical_count" -ge 20 ]; then
  canonical_pass=1
  evidence+=("canonical eval files (individual): ${canonical_count} (≥20 V69 threshold MET)")
else
  failures+=("canonical eval files (individual): ${canonical_count} (need ≥20 for V69 close · batched B104/B105 do NOT count)")
fi

evidence=("${evidence[@]:1}")
failures=("${failures[@]:1}")

# V69 score: mass_balance 35 + corpus 15 + bc_routes 10 + whitelist 20 + canonical 20 = 100
score=$(( massbal_pass * 35 + corpus_pass * 15 + bc_routes_pass * 10 + whitelist_pass * 20 + canonical_pass * 20 ))
...
print(json.dumps({
  ...
  "subscores": {
    ...
    "canonical_eval_count": $canonical_count,
    "canonical_eval_pass": $canonical_pass,
  },
  "evidence": ev,
  "failures": fa,
