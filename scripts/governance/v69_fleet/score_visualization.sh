...
if [ -d "$baseline_dir" ]; then
  ...
  if [ "${png_count:-0}" -ge 18 ]; then
    baseline_score=30
    evidence+=("visual baseline: ${png_count}/18 PNG files (FULL=30/30)")
  elif [ "${png_count:-0}" -gt 0 ]; then
    baseline_score=$(( png_count * 30 / 18 ))
    evidence+=("visual baseline: ${png_count}/18 PNG files (pro-rated=${baseline_score}/30)")
    failures+=("visual baseline incomplete: ${png_count}/18 PNG snapshot files (need ≥18 for V69 close)")
  else
    failures+=("visual baseline empty: 0 PNG files in ${baseline_dir} (need ≥18 for V69 close)")
  fi
else
...
print(json.dumps({
  ...
  "evidence": ev,
  "failures": fa,
  "honest_note": "V69 tightened · ≥16 PNG snapshot files + ≥4 viewport-mode specs required for full score · pro-rated below threshold"
