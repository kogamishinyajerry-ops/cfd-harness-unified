#!/usr/bin/env bash
# V69 Fleet Agent #6: Functional Checklist
# V69 target: 4/4 sub-DECs LANDED + 7/7 Done dims MET
# (charter §5 originally listed 5 sub-DECs; V69.3 CompletenessCard real-data
#  wiring was consolidated into V69.2 because both consume the same
...
landed=0
for f in $(ls "$sub_dec_dir"/2026-05-*_v69_sub_*.md 2>/dev/null); do
...
done_met=0
arc_goal=".planning/V69_ARC_GOAL.md"
if [ -f "$arc_goal" ]; then
  done_met=$(grep -cE "^- \[x\] \*\*V69-DONE-[1-7]" "$arc_goal" 2>/dev/null | head -1 | tr -d ' \n')
  done_met=${done_met:-0}
  evidence+=("Done dims MET: ${done_met}/7 (from $arc_goal)")
else
  failures+=("V69_ARC_GOAL.md not yet authored · Done dims = 0/7 honest baseline")
fi
