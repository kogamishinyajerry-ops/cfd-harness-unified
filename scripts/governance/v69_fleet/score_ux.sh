...
PYEOF
  ...
  if [ "$total" -gt 0 ]; then
    # V69 (further tightened): ≥11 specs PASS for FULL flow_score=60 · pro-rated otherwise
    if [ "$passed" -ge 11 ]; then
      flow_completion_score=60
      evidence+=("playwright: ${passed} specs PASS ≥11 threshold (FULL=60/60)")
    else
      flow_completion_score=$(( passed * 60 / 11 ))
      evidence+=("playwright: ${passed}/11 PASS threshold (pro-rated=${flow_completion_score}/60 · ${passed}/${total} total)")
      failures+=("ux below V69 threshold: ${passed}/11 specs PASS (need ≥11 for V69 close)")
    fi
