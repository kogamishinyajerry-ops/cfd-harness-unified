#!/usr/bin/env bash
# V77 Fleet Aggregator · 16 pillars
# Runs:
#   - V69 7 + V70 3 + V72 interaction_polish (v74_fleet)
#   - V73 backend_integration (v77_fleet · ≥35 threshold)
#   - V74 data_fidelity (v74_fleet)
#   - V75 resumability_observability (v75_fleet)
#   - V76 visualization_fidelity (v76_fleet)
#   - V77 NEW real_time_solver_observability (v77_fleet)
# Usage: bash scripts/governance/v77_fleet/score_all.sh <iter_number>
# Output: .planning/scores/V77_iter_${N}.md

set -o pipefail
cd "$(git rev-parse --show-toplevel)" || exit 1

iter="${1:-0}"
out=".planning/scores/V77_iter_${iter}.md"
mkdir -p "$(dirname "$out")"
...
echo ">>> V77 Fleet · Iter $iter · $ts · commit $sha" >&2

for a in "${agents[@]}"; do
  echo ">>> running agent: $a" >&2
  # V77: real_time_solver_observability NEW + backend_integration extended
  # V76: visualization_fidelity unchanged
  # V75: resumability_observability unchanged
  # V74: data_fidelity / interaction_polish unchanged
  # Else: v71_fleet
  if [ "$a" = "real_time_solver_observability" ] || [ "$a" = "backend_integration" ]; then
...
md = []
md.append(f"# V77 Fleet Score · Iter {iter_n}\n")
...
md.append("## Per-Dim Scores (16 pillars)\n")
...
md.append("- ✓ min() one-vote veto applied across all 16 pillars (no average masking)\n")
