...
if [ -d "$baseline_dir" ]; then
  ...
  if [ "${png_count:-0}" -ge 12 ]; then
    baseline_score=30
    evidence+=("visual baseline: ${png_count}/12 PNG files (FULL=30/30)")
  elif [ "${png_count:-0}" -gt 0 ]; then
    baseline_score=$(( png_count * 30 / 12 ))
    evidence+=("visual baseline: ${png_count}/12 PNG files (pro-rated=${baseline_score}/30)")
    failures+=("visual baseline incomplete: ${png_count}/6 PNG snapshot files (need ≥12 for V68-B.4 close)")
  else
    failures+=("visual baseline empty: 0 PNG files in ${baseline_dir} (need ≥12 for V68-B.4 close)")
  fi
else
...
print(json.dumps({
  ...
  "evidence": ev,
  "failures": fa,
  "honest_note": "V68-B tightened · ≥12 PNG snapshot files + ≥4 viewport-mode specs required for full score · pro-rated below threshold"
