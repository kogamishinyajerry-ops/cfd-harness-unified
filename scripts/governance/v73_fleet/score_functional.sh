#!/usr/bin/env bash
# V73 Fleet Agent #6: Functional Checklist
# V73 target: 6/6 sub-DECs LANDED + 9/9 Done dims MET (expanded from V69 4/7)
...
landed=0
for f in $(ls "$sub_dec_dir"/2026-05-*_v71_sub_*.md 2>/dev/null); do
...
done_met=0
arc_goal=".planning/V73_ARC_GOAL.md"
if [ -f "$arc_goal" ]; then
  done_met=$(grep -cE "^- \[x\] \*\*V73-DONE-[1-9]" "$arc_goal" 2>/dev/null | head -1 | tr -d ' \n')
  done_met=${done_met:-0}
  evidence+=("Done dims MET: ${done_met}/9 (from $arc_goal)")
else
  failures+=("V73_ARC_GOAL.md not yet authored · Done dims = 0/9 honest baseline")
fi
