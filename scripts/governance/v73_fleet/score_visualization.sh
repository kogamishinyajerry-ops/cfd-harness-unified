...
if [ -d "$baseline_dir" ]; then
  ...
  if [ "${png_count:-0}" -ge 30 ]; then
    ...
    failures+=("visual baseline incomplete: ${png_count}/30 PNG snapshot files (need ≥30 for V73 close)")
  else
    failures+=("visual baseline empty: 0 PNG files in ${baseline_dir} (need ≥30 for V73 close)")
  fi
else
...
print(json.dumps({
  ...
  "evidence": ev,
  "failures": fa,
  "honest_note": "V73 tightened · ≥30 PNG snapshot files + ≥4 viewport-mode specs required for full score · pro-rated below threshold"
