...
PYEOF
  ...
  if [ "$total" -gt 0 ]; then
    # V73 (further tightened): ≥13 specs PASS for FULL flow_score=60 · pro-rated otherwise
    if [ "$passed" -ge 17 ]; then
      flow_completion_score=60
      evidence+=("playwright: ${passed} specs PASS ≥17 threshold (FULL=60/60)")
    else
      flow_completion_score=$(( passed * 60 / 17 ))
      evidence+=("playwright: ${passed}/17 PASS threshold (pro-rated=${flow_completion_score}/60 · ${passed}/${total} total)")
      failures+=("ux below V70 threshold: ${passed}/17 specs PASS (need ≥17 for V73 close · was ≥13 for V70)")
    fi
...
print(json.dumps({
  ...
  "evidence": ev,
  "failures": fa,
  "honest_note": "V69 tightened · ≥9 specs PASS for FULL · pro-rated below"
