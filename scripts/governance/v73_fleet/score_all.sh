#!/usr/bin/env bash
# V73 Fleet Aggregator
# Runs 12 score agents:
#   V69 7 + V70 3 + V72 NEW interaction_polish + V73 NEW backend_integration
# Collects JSON, writes markdown report.
# Usage: bash scripts/governance/v73_fleet/score_all.sh <iter_number>
# Output: .planning/scores/V73_iter_${N}.md

set -o pipefail
cd "$(git rev-parse --show-toplevel)" || exit 1

iter="${1:-0}"
out=".planning/scores/V73_iter_${iter}.md"
mkdir -p "$(dirname "$out")"

ts=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
sha=$(git rev-parse HEAD)

agents=(quality physics ux visualization smoke functional stability cfd_breadth novice_onboarding industrial_ui interaction_polish backend_integration)
results_dir=$(mktemp -d)
trap "rm -rf $results_dir" EXIT

echo ">>> V73 Fleet · Iter $iter · $ts · commit $sha" >&2

for a in "${agents[@]}"; do
  echo ">>> running agent: $a" >&2
  # V73: backend_integration uses v73_fleet/ script (NEW pillar 12);
  # interaction_polish uses v72_fleet/ (V72 pillar 11);
  # everything else reuses v71_fleet/ scripts
  # V73: backend_integration AND interaction_polish use v73_fleet/ (extended);
  # everything else reuses v71_fleet/ scripts
  if [ "$a" = "backend_integration" ] || [ "$a" = "interaction_polish" ]; then
    script_path="scripts/governance/v73_fleet/score_${a}.sh"
  else
...
md = []
md.append(f"# V73 Fleet Score · Iter {iter_n}\n")
md.append(f"**Generated**: {ts}  ")
md.append(f"**Commit**: `{sha}`  ")
md.append(f"**Total (min one-vote-veto across 12 pillars)**: **{min_score} / 100**  ")
...
md.append("## Per-Dim Scores (12 pillars)\n")
...
md.append("- ✓ min() one-vote veto applied across all 12 pillars (no average masking)\n")
