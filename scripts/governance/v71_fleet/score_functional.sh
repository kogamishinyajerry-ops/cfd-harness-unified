#!/usr/bin/env bash
# V71 Fleet Agent #6: Functional Checklist
# V71 target: 6/6 sub-DECs LANDED + 9/9 Done dims MET (expanded from V69 4/7)
...
done_met=0
arc_goal=".planning/V71_ARC_GOAL.md"
if [ -f "$arc_goal" ]; then
  done_met=$(grep -cE "^- \[x\] \*\*V71-DONE-[1-9]" "$arc_goal" 2>/dev/null | head -1 | tr -d ' \n')
  done_met=${done_met:-0}
  evidence+=("Done dims MET: ${done_met}/9 (from $arc_goal)")
else
  failures+=("V71_ARC_GOAL.md not yet authored · Done dims = 0/9 honest baseline")
fi
