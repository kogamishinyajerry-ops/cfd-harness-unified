#!/usr/bin/env bash
# V71 Fleet Aggregator
# Runs 10 score agents (V69 7 + V70 3 new), collects JSON, writes markdown report.
# Usage: bash scripts/governance/v71_fleet/score_all.sh <iter_number>
# Output: .planning/scores/V71_iter_${N}.md

set -o pipefail
cd "$(git rev-parse --show-toplevel)" || exit 1

iter="${1:-0}"
out=".planning/scores/V71_iter_${iter}.md"
mkdir -p "$(dirname "$out")"
...
echo ">>> V71 Fleet · Iter $iter · $ts · commit $sha" >&2

for a in "${agents[@]}"; do
  echo ">>> running agent: $a" >&2
  bash "scripts/governance/v71_fleet/score_${a}.sh" > "$results_dir/${a}.json" 2> "$results_dir/${a}.stderr.log"
...
md = []
md.append(f"# V71 Fleet Score · Iter {iter_n}\n")
