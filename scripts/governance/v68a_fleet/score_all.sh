#!/usr/bin/env bash
# V68-A Fleet Aggregator
# Runs 7 score agents, collects JSON, writes markdown report.
# Usage: bash scripts/governance/v68a_fleet/score_all.sh <iter_number>
# Output: .planning/scores/V68-A_iter_${N}.md

set -o pipefail
cd "$(git rev-parse --show-toplevel)" || exit 1

iter="${1:-0}"
out=".planning/scores/V68-A_iter_${iter}.md"
mkdir -p "$(dirname "$out")"
...
echo ">>> V68-A Fleet · Iter $iter · $ts · commit $sha" >&2

for a in "${agents[@]}"; do
  echo ">>> running agent: $a" >&2
  bash "scripts/governance/v68a_fleet/score_${a}.sh" > "$results_dir/${a}.json" 2> "$results_dir/${a}.stderr.log"
...
md = []
md.append(f"# V68-A Fleet Score · Iter {iter_n}\n")
