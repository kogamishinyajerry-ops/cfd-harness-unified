...
if [ -d "$baseline_dir" ]; then
  ...
  if [ "${png_count:-0}" -ge 6 ]; then
    baseline_score=30
    evidence+=("visual baseline: ${png_count}/6 PNG files (FULL=30/30)")
  elif [ "${png_count:-0}" -gt 0 ]; then
    baseline_score=$(( png_count * 30 / 6 ))
    evidence+=("visual baseline: ${png_count}/6 PNG files (pro-rated=${baseline_score}/30)")
    failures+=("visual baseline incomplete: ${png_count}/6 PNG snapshot files (need ≥6 for V68-A.4 close)")
  else
    failures+=("visual baseline empty: 0 PNG files in ${baseline_dir} (need ≥6 for V68-A.4 close)")
  fi
else
...
print(json.dumps({
  ...
  "evidence": ev,
  "failures": fa,
  "honest_note": "V68-A tightened · ≥6 PNG snapshot files + ≥4 viewport-mode specs required for full score · pro-rated below threshold"
