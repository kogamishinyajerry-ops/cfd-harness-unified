...
PYEOF
  ...
  if [ "$total" -gt 0 ]; then
    # V68-A: ≥5 specs PASS for FULL flow_score=60 · pro-rated otherwise
    if [ "$passed" -ge 5 ]; then
      flow_completion_score=60
      evidence+=("playwright: ${passed} specs PASS ≥5 threshold (FULL=60/60)")
    else
      flow_completion_score=$(( passed * 60 / 5 ))
      evidence+=("playwright: ${passed}/5 PASS threshold (pro-rated=${flow_completion_score}/60 · ${passed}/${total} total)")
      failures+=("ux below threshold: ${passed}/5 specs PASS (need ≥5 for V68-A close)")
    fi
...
print(json.dumps({
  ...
  "evidence": ev,
  "failures": fa,
  "honest_note": "V68-A tightened · ≥5 specs PASS for FULL flow_completion=60 · pro-rated below"
