#!/usr/bin/env bash
# V72 Fleet Aggregator
# Runs 11 score agents (V69 7 + V70 3 + V72 NEW interaction_polish), collects JSON,
# writes markdown report.
# Usage: bash scripts/governance/v72_fleet/score_all.sh <iter_number>
# Output: .planning/scores/V72_iter_${N}.md

set -o pipefail
cd "$(git rev-parse --show-toplevel)" || exit 1

iter="${1:-0}"
out=".planning/scores/V72_iter_${iter}.md"
mkdir -p "$(dirname "$out")"

ts=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
sha=$(git rev-parse HEAD)

agents=(quality physics ux visualization smoke functional stability cfd_breadth novice_onboarding industrial_ui interaction_polish)
results_dir=$(mktemp -d)
trap "rm -rf $results_dir" EXIT

echo ">>> V72 Fleet · Iter $iter · $ts · commit $sha" >&2

for a in "${agents[@]}"; do
  echo ">>> running agent: $a" >&2
  # V72: interaction_polish uses v72_fleet/ script (new pillar); others reuse v71_fleet/ scripts
  if [ "$a" = "interaction_polish" ]; then
    script_path="scripts/governance/v72_fleet/score_${a}.sh"
  else
...
md = []
md.append(f"# V72 Fleet Score · Iter {iter_n}\n")
md.append(f"**Generated**: {ts}  ")
md.append(f"**Commit**: `{sha}`  ")
md.append(f"**Total (min one-vote-veto across 11 pillars)**: **{min_score} / 100**  ")
...
md.append("## Per-Dim Scores (11 pillars)\n")
...
md.append("- ✓ min() one-vote veto applied across all 11 pillars (no average masking)\n")
