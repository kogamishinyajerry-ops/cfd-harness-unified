#!/usr/bin/env bash
# V72 Fleet Agent #10: Industrial-UI-Benchmark (extended)
...
#   - v71_ui_tags (12)              · ≥6 V72-UI tagged components/files
#   - v71_baselines (16)            · ≥8 V72 baselines (numbered 23-30)
...
if [ "$v3_routes" -ge 2 ]; then
  v3_route_score=16
  evidence+=("v3 route mounts: ${v3_routes} (≥2 V72 threshold MET · /workbench/v3 + WorkbenchShellV3)")
elif [ "$v3_routes" -gt 0 ]; then
  v3_route_score=$(( v3_routes * 16 / 2 ))
  evidence+=("v3 route mounts: ${v3_routes}/2 (pro-rated=${v3_route_score}/16)")
  failures+=("v3 route mounts below threshold: ${v3_routes}/2 (need ≥2 for V72-DONE-1)")
else
  failures+=("v3 route mounts: 0 (need /workbench/v3 route + WorkbenchShellV3 component for V72-DONE-1)")
fi

# 6. V72-UI tags (12)
v71_ui_tags=0
if [ -d "ui/frontend/src" ]; then
  v71_ui_tags=$(grep -rohE "V72.UI|V72-UI|V72.SHELL|V72-SHELL" ui/frontend/src/ 2>/dev/null | wc -l | tr -d ' ')
fi
v71_ui_tags=${v71_ui_tags:-0}
if [ "$v71_ui_tags" -ge 6 ]; then
  v71_ui_score=12
  evidence+=("V72-UI tags in code: ${v71_ui_tags} (≥6 V72 threshold MET)")
elif [ "$v71_ui_tags" -gt 0 ]; then
  v71_ui_score=$(( v71_ui_tags * 12 / 6 ))
  failures+=("V72-UI tags below threshold: ${v71_ui_tags}/6")
else
  v71_ui_score=0
  failures+=("V72-UI tags: 0 (need ≥6 V72-UI/V72-SHELL tags in code for V72-DONE-9)")
fi

# 7. V72 baselines 23-30 (16)
v71_baselines=0
...
if [ "$v71_baselines" -ge 8 ]; then
  ...
  evidence+=("V72 baselines (23-30): ${v71_baselines}/8 (V72-DONE-7 MET)")
elif [ "$v71_baselines" -gt 0 ]; then
  v71_baseline_score=$(( v71_baselines * 16 / 8 ))
  failures+=("V72 baselines: ${v71_baselines}/8 (need ≥8 for V72-DONE-7)")
else
  v71_baseline_score=0
  failures+=("V72 baselines: 0/8 (need ≥8 for V72-DONE-7 · numbered 23-30)")
fi
...
print(json.dumps({
  ...
  "evidence": ev,
  "failures": fa,
  "honest_note": "V72 industrial-UI agent tightened with v3 blueprint compliance subscores · still enforces V70 anti-marketing gate · expects v3 route + 6 V72-UI tags + 8 baselines + blueprint INDEX + palette compliance"
