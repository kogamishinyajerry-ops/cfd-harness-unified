...
PYEOF
  ...
  if [ "$total" -gt 0 ]; then
    # V72 (further tightened): ≥13 specs PASS for FULL flow_score=60 · pro-rated otherwise
    if [ "$passed" -ge 17 ]; then
      ...
      failures+=("ux below V70 threshold: ${passed}/17 specs PASS (need ≥17 for V72 close · was ≥13 for V70)")
    fi
