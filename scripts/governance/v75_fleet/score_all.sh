#!/usr/bin/env bash
# V75 Fleet Aggregator
# Runs 14 score agents:
#   V69 7 + V70 3 + V72 interaction_polish + V73 backend_integration + V74 data_fidelity + V75 NEW resumability_observability
# Collects JSON, writes markdown report.
# Usage: bash scripts/governance/v75_fleet/score_all.sh <iter_number>
# Output: .planning/scores/V75_iter_${N}.md

set -o pipefail
cd "$(git rev-parse --show-toplevel)" || exit 1

iter="${1:-0}"
out=".planning/scores/V75_iter_${iter}.md"
mkdir -p "$(dirname "$out")"

ts=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
sha=$(git rev-parse HEAD)

agents=(quality physics ux visualization smoke functional stability cfd_breadth novice_onboarding industrial_ui interaction_polish backend_integration data_fidelity resumability_observability)
results_dir=$(mktemp -d)
trap "rm -rf $results_dir" EXIT

echo ">>> V75 Fleet · Iter $iter · $ts · commit $sha" >&2

for a in "${agents[@]}"; do
  echo ">>> running agent: $a" >&2
  # V75: resumability_observability (NEW pillar 14) uses v75_fleet/
  # backend_integration uses v75_fleet/ (extended threshold ≥24)
  # data_fidelity uses v74_fleet/ (unchanged from V74)
  # interaction_polish uses v74_fleet/ (unchanged 5-surface threshold from V74)
  # everything else reuses v71_fleet/ scripts
  if [ "$a" = "resumability_observability" ] || [ "$a" = "backend_integration" ]; then
...
md = []
md.append(f"# V75 Fleet Score · Iter {iter_n}\n")
md.append(f"**Generated**: {ts}  ")
md.append(f"**Commit**: `{sha}`  ")
md.append(f"**Total (min one-vote-veto across 14 pillars)**: **{min_score} / 100**  ")
...
md.append("## Per-Dim Scores (14 pillars)\n")
...
md.append("- ✓ min() one-vote veto applied across all 14 pillars (no average masking)\n")
