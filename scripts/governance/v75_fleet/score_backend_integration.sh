...
wired_score=0
# V75 threshold: ≥24 (was ≥18 in V74) · forces observability + skeleton
# query refs to land beyond V74's per-ref completeness work.
if [ "${wired:-0}" -ge 24 ]; then
  wired_score=40
  evidence+=("real-wired surfaces: ${wired} useQuery/api refs (FULL=40/40)")
elif [ "${wired:-0}" -gt 0 ]; then
  wired_score=$(( wired * 40 / 24 ))
  if [ "$wired_score" -gt 40 ]; then wired_score=40; fi
  evidence+=("real-wired surfaces: ${wired}/24 (pro-rated=${wired_score}/40)")
else
