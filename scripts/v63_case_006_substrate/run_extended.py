"""Path (b) extended · assemble_stack on case_006 with the 3 newly synthesized inputs.

DEC-V63-A-sub-M-CASE-006-SUBSTRATE (V63-A Tier 2 · carry-over #3 from
TRACK-3-rerun §V-row truth-capture rate). Mirrors
``scripts/stack_track_c_session_3_rerun/run_python_path.py`` (same parts_manifest +
shm_dict + thermo_dict + step_path) and adds three substrate-side input
files newly landed under ``case_006/inputs/``:

  - ``thin_wall_inputs.yaml`` -> ``thin_wall_advisor`` dispatch (V10/V12/V94)
  - ``interface_bodies.json`` + ``interface_specs.json`` -> A2-v2
    ``virtual_interface_detector`` dispatch (V20/V32/V47/V61)

Goal: push V-row truth-capture rate from 1/9 (V29 only, post-D10 LANDED) to
>=3/9 (add V30 thin_wall sliver + D1 sub-mm gap).

Run from repo root::

    .venv/bin/python -m scripts.v63_case_006_substrate.run_extended
"""
...
from ui.backend.services.geometry_ingest import virtual_interface_detector  # noqa: E402
from ui.backend.services.geometry_ingest import thin_wall_advisor  # noqa: E402

INPUTS = CASE_DIR / "inputs"


def load_thin_wall_inputs() -> dict:
    """Load case_006/inputs/thin_wall_inputs.yaml and rebuild PatchGeometry tuples."""
...
def load_interface_bodies() -> dict[str, virtual_interface_detector.BodyGeometry]:
    """Load case_006/inputs/interface_bodies.json and rebuild BodyGeometry dataclasses."""
...
def load_interface_specs() -> tuple[virtual_interface_detector.InterfaceSpec, ...]:
    """Load case_006/inputs/interface_specs.json -> InterfaceSpec tuple."""
