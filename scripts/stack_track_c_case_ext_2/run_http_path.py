"""Path (a) · ``POST /api/ai-review`` via FastAPI TestClient against case_009.
...
Run from repo root:
    .venv/bin/python -m scripts.stack_track_c_case_ext_2.run_http_path
"""
...
def main() -> dict:
    # Symmetric with TRACK-3 case_006 + M-CASE-EXT-1 case_004: pass only
    # parts_manifest to avoid auto-discovery confounds. shm_dict / thermo_dict
    # are None (build_* returned None for case_009 v1 substrate via the
    # python-path; the HTTP route does not currently accept them as fields
    # of AIReviewRequest anyway — same route-schema gap noted in TRACK-1
    # §4 / TRACK-2 §7 / TRACK-3 §7 / M-CASE-EXT-1 §3.3, now 5th recurrence).
    payload = {
        "parts_manifest": build_parts_manifest(),
    }

    payload_path = Path(__file__).parent / "case_009_v1_payload.json"
