"""Path (b) rerun: Direct import of assemble_stack() for case_011 v5b
after V99-WIDEN (commit 13e58b8) + REQ-SCHEMA-EXPAND (commit a1119ae).

Reuses the same input dicts as TRACK-1 original
(scripts/stack_track_c_session_1/build_inputs.py) so the rerun is a
controlled experiment: only the advisor/route source changed, not the
case under test.

Output: stack_report_python_rerun.json with full AdvisorStackReport
serialization.
...
def main() -> int:
    ...
    out_path = Path(__file__).parent / "stack_report_python_rerun.json"
    out_path.write_text(json.dumps(dataclasses.asdict(report), indent=2, default=_serialize))

    print("=== stack report (python path RERUN · V99-WIDEN + REQ-SCHEMA-EXPAND) ===")
