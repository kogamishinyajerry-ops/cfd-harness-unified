"""DEC-V61-062 Stage E live-run driver: NACA0012 at α∈{0°, 4°, 8°}.
...
DEC-V61-062 extractors, and emits:

  reports/phase5_audit/dec_v61_062_live_run_<timestamp>_alpha<N>.json
    raw stdout/stderr per α
  reports/phase5_audit/dec_v61_062_live_summary.yaml
...
Usage:
    .venv/bin/python scripts/dec_v61_062_live_runs.py
"""
...
def _per_run(alpha_deg: float, timestamp: str) -> dict:
    ...
    raw_path = REPORTS_DIR / (
        f"dec_v61_062_live_run_{timestamp}_alpha{int(alpha_deg)}.json"
    )
    raw_path.write_text(
        json.dumps(
            {
...
def _aggregate(records: list, timestamp: str) -> dict:
    ...
    out_path = REPORTS_DIR / "dec_v61_062_live_summary.yaml"
...
def main() -> int:
    ...
    print(f"DEC-V61-062 Stage E live-run driver · timestamp={timestamp}", flush=True)
    records = []
