...
~<duration> from now (cadence: 6h, /loop dependent on Claude Code
session being open).
```
...
To stop the loop: send `/loop stop` or end the Claude Code session
...
- `.claude/skills/cfd-harness-harvest/SKILL.md` — sibling skill for
