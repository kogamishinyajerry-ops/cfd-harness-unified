...
| `src.metrics.*` | MetricsRegistry + 4 metric class wrappers (P1-T1 arc clean-close DEC-V61-054) + TrustGate overall-verdict reducer (P1-T2) per METRICS_AND_TRUST_GATES v0.1 Draft |
...
> **Update 2026-04-25 (W2 Impl Start · commit 3e9205f) · ADR-001 AC-A7
> closure**: hand-enumerated source_modules / forbidden_modules
> brittleness is **closed** by the ADR-002 PLANE_OF SSOT mechanism
> shipped today. Authoritative plane assignment now lives in
> `src/_plane_assignment.py` (Plane enum + PLANE_OF dict);
> `.importlinter` is mechanically generated from that table by
> `scripts/gen_importlinter.py`, with a `--check` step in CI
> (`PLANE_OF SSOT byte-identical check`) that fails the build on
> any drift. `tests/test_plane_assignment_ssot.py` now executes the
> AC-2 orphan / ghost coverage check (was previously listed as
> "implementation pending" in this section). Result: any new
> top-level `src.*` package added without a corresponding
> `PLANE_OF` entry fails CI before `lint-imports` even runs, and
> any drift between PLANE_OF and `.importlinter` is structurally
> impossible at merge time. Opus round-1 Additional Concern #1
> (cross-reference between ADR-001 §2.7 and ADR-002 SSOT ownership)
> is satisfied by this update.
