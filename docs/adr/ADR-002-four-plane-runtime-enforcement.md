...
| W4 hard-fail toggle | **Gate #1 trigger** (Gate #3 closes — see OPS-2026-04-25-001 §7) | After dogfood signal review (zero dedup'd violations + escape rate <20% + ≥30 backend pytest CI runs + ≥15 cross-track commits): CI `backend-tests` WARN-mode pytest step flips to required-passing (set `continue-on-error: false`); src/__init__.py default may stay "off" (W5 separates default flip). Rollback trigger (§4.1 A6) armed via `plane_guard_rollback_cron.yml`. **Calendar form "2026-05-09 → 2026-05-11" deprecated 2026-04-25T17:30** per Opus 4.7 ACCEPT_WITH_COMMENTS 9-gates refactor (RETRO-V61-006 Addendum 5). |
| W5 ON-everywhere | **Gate #2 trigger** (W4 merged ∧ ≥10 consecutive backend pytest CI runs all green ∧ A13/A18 dedup'd incidents = 0 ∧ line A SOLE escape-tag commits = 0 in those 10) | Default env var `"warn"` (not yet `"on"` until post-W5-1 stage triggers — see `.planning/roadmaps/post_w5_roadmap.md`) in `src/__init__.py`; P2 ExecutorMode refactor unblocked (meets §2.3 P2 startup-PR precondition). **Calendar form "2026-05-19" deprecated 2026-04-25T17:30**. |
